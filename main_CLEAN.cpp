...
int main() {
    ...
    Chimera::CTraderFIXClient fix_client;
    fix_client.setConfig(fix_config);

    // Set tick callback - feed to executor
    fix_client.setOnTick([&](const Chimera::CTraderTick& ctick) {
    ...
    for (int i = 0; i < 50 && fix_client.getState() != Chimera::CTraderState::RUNNING; ++i) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    if (fix_client.getState() == Chimera::CTraderState::RUNNING) {
