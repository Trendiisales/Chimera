...
// =============================================================================
// v7.08: Added per-symbol tick tracking for starvation detection
// =============================================================================
...
// =============================================================================
// PER-SYMBOL TICK TRACKER - v7.08
// =============================================================================
struct SymbolTickStats {
    std::atomic<uint64_t> ticks{0};
    std::atomic<uint64_t> first_tick_ms{0};
    std::atomic<uint64_t> last_tick_ms{0};
    std::atomic<uint64_t> subscribed_at_ms{0};
...
public:
    ...
    bool connect() {
        ...
        // Reset QUOTE start flag for this connection attempt
        quoteStarted_.store(false);
        ...
        tradeSession_.setOnLogon([this]() {
            ...
            // NOW start QUOTE session (only after TRADE is confirmed)
            // UNLESS we're in single-session mode
            if (config_.singleSessionMode) {
                fprintf(stderr, "[FIX-DBG][CTraderFIXClient] SINGLE-SESSION MODE - skipping QUOTE, using TRADE for market data\n");
                fflush(stderr);
                std::cout << "[CTraderFIX] Single-session mode: using TRADE for both orders and market data\n";
                state_.store(CTraderState::RUNNING);
                notifyState();
                return;
            }
            
            if (!quoteStarted_.exchange(true)) {  // Atomic test-and-set
                fprintf(stderr, "[FIX-DBG][CTraderFIXClient] Starting QUOTE from TRADE onLogon...\n");
                fflush(stderr);
                std::cout << "[CTraderFIX] Starting QUOTE session (TRADE active)...\n";
                std::cout << "[CTraderFIX] Waiting 1 second before QUOTE logon...\n";
                std::this_thread::sleep_for(std::chrono::seconds(1));  // Give broker time to settle TRADE
                state_.store(CTraderState::CONNECTING_QUOTE);
                
                if (!quoteSession_.start(config_.host, config_.pricePort, false)) {  // Don't reset sequences - try without 141 tag
                    fprintf(stderr, "[FIX-DBG][CTraderFIXClient] QUOTE start FAILED from callback\n");
                    fflush(stderr);
                    std::cerr << "[CTraderFIX] WARNING: QUOTE session failed to start\n";
                }
            }
        });
        ...
        fflush(stderr);
        std::cout << "[CTraderFIX] TRADE logon OK\n";
        std::cout << "[CTraderFIX] QUOTE will start from TRADE onLogon callback\n";
        std::cout << "[CTraderFIX] ============================================\n";
    ...
    bool requestSecurityList() {
        bool sessionOK = config_.singleSessionMode ? tradeConnected_.load() : quoteConnected_.load();
        
        if (!sessionOK) {
            std::cerr << "[CTraderFIX] Cannot request security list: " 
                      << (config_.singleSessionMode ? "TRADE" : "QUOTE") 
                      << " not connected\n";
            return false;
        }
        ...
        std::cout << "[CTraderFIX] Requesting security list on " 
                  << (config_.singleSessionMode ? "TRADE" : "QUOTE") << " session...\n";
        
        if (config_.singleSessionMode) {
            return tradeSession_.sendSecurityListRequest();
        } else {
    ...
    // v4.9.34: FIX Session RTT (co-located latency from TestRequest/Heartbeat)
    double fixRttLastMs() const noexcept { return tradeSession_.rttLastMs(); }
    double fixRttMinMs() const noexcept { return tradeSession_.rttMinMs(); }
    double fixRttMaxMs() const noexcept { return tradeSession_.rttMaxMs(); }
    double fixRttAvgMs() const noexcept { return tradeSession_.rttAvgMs(); }
    size_t fixRttSamples() const noexcept { return tradeSession_.rttSamples(); }
    ...
    bool subscribeMarketData(const std::string& symbol) {
        // In single-session mode, use TRADE for market data
        bool sessionOK = config_.singleSessionMode ? tradeConnected_.load() : quoteConnected_.load();
        
        if (!sessionOK) {
            std::cerr << "[CTraderFIX] Cannot subscribe: " 
                      << (config_.singleSessionMode ? "TRADE" : "QUOTE") 
                      << " not connected\n";
            return false;
        }
        ...
        // =================================================================
        // Track subscription - v7.08
        // =================================================================
        uint64_t now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now().time_since_epoch()).count();
        symbolTickStats_[normalized].subscribed = true;
        symbolTickStats_[normalized].subscribed_at_ms.store(now_ms);
        ...
        std::cout << "[CTraderFIX] ✓ Subscribing to " << normalized 
                  ...
                  << " on " << (config_.singleSessionMode ? "TRADE" : "QUOTE") << " session\n";
        
        // Send on appropriate session
        if (config_.singleSessionMode) {
            return tradeSession_.sendMarketDataRequest(securityIdStr, true);
        } else {
    ...
    bool sendMarketOrder(const std::string& symbol, char side, double qty,
        ...
        // ═══════════════════════════════════════════════════════════════════
        // v4.18.0: EXECUTION AUTHORITY — checks on HUMAN symbol, BEFORE
        // numeric conversion. This is the correct architectural layer.
        // ═══════════════════════════════════════════════════════════════════
        if (!checkExecutionAuthority(symbol)) {
            return false;
        }
        
        // v7.10 FIX: cTrader requires NUMERIC symbol ID, not name string
        std::string symbolId = resolveSymbolId(symbol);
        
        // v4.18.0: Clamp volume to broker constraints
        double clampedQty = clampVolume(symbol, qty);
        ...
        // v4.2.2: Track order for latency measurement
        uint64_t sent_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::steady_clock::now().time_since_epoch()).count();
        
        std::string clOrdID = tradeSession_.sendNewOrder(symbolId, side, clampedQty, FIXOrdType::Market, 0.0, positionEffect);
        
        if (!clOrdID.empty()) {
            std::lock_guard<std::mutex> lock(orderMtx_);
            pendingOrders_[clOrdID] = {symbol, expected_price, sent_ns};
        }
    ...
    bool sendLimitOrder(const std::string& symbol, char side, double qty, double price,
        ...
        // ═══════════════════════════════════════════════════════════════════
        // v4.18.0: EXECUTION AUTHORITY — checks on HUMAN symbol
        // ═══════════════════════════════════════════════════════════════════
        if (!checkExecutionAuthority(symbol)) {
            return false;
        }
        ...
        std::string clOrdID = tradeSession_.sendNewOrder(symbolId, side, clampedQty, FIXOrdType::Limit, price, positionEffect);
        
        if (!clOrdID.empty()) {
            std::lock_guard<std::mutex> lock(orderMtx_);
            pendingOrders_[clOrdID] = {symbol, price, sent_ns};
        }
...
private:
    // =========================================================================
    // v4.18.0: EXECUTION AUTHORITY — operates on HUMAN symbol names
    // Called BEFORE numeric symbol conversion. Proper architectural layer.
    // =========================================================================
    bool checkExecutionAuthority(const std::string& symbol) {
        bool intent_live = tradeSession_.isIntentLive();
        bool fix_connected = tradeConnected_.load();
        bool expansion = tradeSession_.isNYExpansion();
        
        ExecBlockReason block_reason;
        if (!getExecutionAuthority().allowCFD(symbol.c_str(), fix_connected, expansion, intent_live, &block_reason)) {
            std::cout << "\n╔══════════════════════════════════════════════════════════╗\n";
            std::cout << "║  ⛔ ORDER BLOCKED - EXECUTION AUTHORITY                   ║\n";
            std::cout << "║  Symbol: " << symbol << "\n";
            std::cout << "║  Reason: " << execBlockReasonToString(block_reason) << "\n";
            std::cout << "║  Intent Live: " << (intent_live ? "YES" : "NO") << "\n";
            std::cout << "║  FIX Connected: " << (fix_connected ? "YES" : "NO") << "\n";
            std::cout << "║  NY Expansion: " << (expansion ? "YES" : "NO") << "\n";
            std::cout << "╚══════════════════════════════════════════════════════════╝\n\n";
            return false;
        }
        
        // Global risk governor
        if (!GlobalRiskGovernor::instance().canSubmitOrder(EngineId::CFD)) {
            std::cout << "[CTraderFIX] ORDER BLOCKED - RISK GOVERNOR (daily loss or throttle)\n";
            return false;
        }
        
        // NAS100 ownership
        if (symbol == "NAS100" && !canTradeNAS100(EngineId::CFD)) {
            std::cout << "[CTraderFIX] ORDER BLOCKED - NAS100 OWNERSHIP VIOLATION\n";
            return false;
        }
    ...
    // =========================================================================
    // v4.18.0: Resolve human symbol name to cTrader numeric ID
    // =========================================================================
    std::string resolveSymbolId(const std::string& symbol) {
        std::lock_guard<std::mutex> lock(securityMtx_);
        std::string normalized = normalizeSymbol(symbol);
        auto it = symbolToId_.find(normalized);
        if (it != symbolToId_.end()) {
            std::string id = std::to_string(it->second);
            std::cout << "[CTraderFIX] Symbol " << symbol << " -> ID " << id << "\n";
            return id;
        }
    ...
    void handleTradeMessage(const FIXMessage& msg) {
        ...
        switch (msgType) {
            ...
            case FIXMsgType::MarketDataSnapshot:
                // In single-session mode, TRADE session receives market data
                if (config_.singleSessionMode) {
                    handleMarketData(msg);
                }
                break;
            ...
            case 'y':  // SecurityList
                // In single-session mode, TRADE session receives security list
                if (config_.singleSessionMode) {
                    handleSecurityList(msg);
                }
                break;
    ...
    void handleMarketData(const FIXMessage& msg) {
        ...
        if (tick.bid > 0 && tick.ask > 0 && onTick_) {
            // v7.08: Track per-symbol tick stats
            uint64_t now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now().time_since_epoch()).count();
            auto& stats = symbolTickStats_[tick.symbol];
            uint64_t count = stats.ticks.fetch_add(1) + 1;
            if (count == 1) {
                stats.first_tick_ms.store(now_ms);
                std::cout << "[CTraderFIX] 🎯 First tick for " << tick.symbol 
                          << " @ " << now_ms << " ms\n";
            }
            stats.last_tick_ms.store(now_ms);
    ...
    void handleExecutionReport(const FIXMessage& msg) {
        ...
        // v4.2.2: Calculate latency if this is a fill
        if (report.isFill() && onLatency_) {
            ...
            {
                std::lock_guard<std::mutex> lock(orderMtx_);
                auto it = pendingOrders_.find(report.clOrdID);
                if (it != pendingOrders_.end()) {
                    symbol = it->second.symbol;
                    expected_price = it->second.expected_price;
                    sent_ns = it->second.sent_ns;
                    pendingOrders_.erase(it);
                }
            }
            
            if (sent_ns > 0) {
                double rtt_ms = static_cast<double>(recv_ns - sent_ns) / 1'000'000.0;
                
                // Calculate slippage (only if expected_price was set)
                double slippage_bps = 0.0;
                if (expected_price > 0.0 && report.avgPx > 0.0) {
                    slippage_bps = std::abs(report.avgPx - expected_price) / expected_price * 10000.0;
                }
                
                std::cout << "[CTraderFIX] LATENCY: " << symbol 
                          << " RTT=" << std::fixed << std::setprecision(2) << rtt_ms << "ms"
                          << " slip=" << std::setprecision(1) << slippage_bps << "bps\n";
                
                onLatency_(symbol, rtt_ms, slippage_bps);
            }
        }
...
public:
    // =========================================================================
    // SYMBOL TICK STATS ACCESS - v7.08
    // =========================================================================
    const std::unordered_map<std::string, SymbolTickStats>& getSymbolTickStats() const {
        return symbolTickStats_;
    }
    ...
    void printSymbolTickStats() const {
        ...
        std::cout << "\n[FIX-TICK-STATS] ═══════════════════════════════════════════\n";
        for (const auto& kv : symbolTickStats_) {
            const std::string& sym = kv.first;
            const SymbolTickStats& s = kv.second;
            
            uint64_t ticks = s.ticks.load();
            uint64_t last = s.last_tick_ms.load();
            uint64_t age_ms = (last > 0) ? (now_ms - last) : 0;
            double rate = 0.0;
            
            if (s.first_tick_ms.load() > 0 && ticks > 1) {
                double elapsed_sec = (last - s.first_tick_ms.load()) / 1000.0;
                if (elapsed_sec > 0) rate = ticks / elapsed_sec;
            }
            
            const char* status = "🟢 OK";
            if (ticks == 0) status = "🔴 ZERO";
            else if (age_ms > 30000) status = "🔴 STARVED";
            else if (age_ms > 5000) status = "🟡 STALE";
            
            printf("[FIX-TICK-STATS] %-10s | ticks=%6lu | rate=%.2f/s | age=%5lu ms | %s\n",
                   sym.c_str(), ticks, rate, age_ms, status);
        }
        std::cout << "[FIX-TICK-STATS] ═══════════════════════════════════════════\n\n";
    }
    
private:
    // =========================================================================
    // v4.18.0: SYMBOL VOLUME SPECS — Per-symbol min/step/max from broker
    // =========================================================================
    void initSymbolSpecs() {
        // BlackBull / cTrader demo defaults
        // Metals: lot = 1 troy ounce, min 1.00 lot
        symbolSpecs_["XAUUSD"] = {1.0,  100.0,  1.0};
        symbolSpecs_["XAGUSD"] = {1.0,  100.0,  1.0};
        
        // Indices: lot = 1 contract, min 1.00 lot
        symbolSpecs_["NAS100"]  = {1.0,  100.0,  1.0};
        symbolSpecs_["US30"]    = {1.0,  100.0,  1.0};
        symbolSpecs_["US100"]   = {1.0,  100.0,  1.0};
        symbolSpecs_["US500"]   = {1.0,  100.0,  1.0};
        symbolSpecs_["SPX500"]  = {1.0,  100.0,  1.0};
        
        // FX: micro lots allowed (0.01 = 1000 units)
        symbolSpecs_["EURUSD"]  = {0.01, 100.0,  0.01};
        symbolSpecs_["GBPUSD"]  = {0.01, 100.0,  0.01};
        symbolSpecs_["USDJPY"]  = {0.01, 100.0,  0.01};
        symbolSpecs_["AUDUSD"]  = {0.01, 100.0,  0.01};
        
        std::cout << "[CTraderFIX] Symbol specs initialized ("
                  << symbolSpecs_.size() << " symbols)\n";
    }
    
    // Clamp quantity to broker constraints, returns clamped qty (0.0 if invalid)
    double clampVolume(const std::string& symbol, double qty) const {
        auto it = symbolSpecs_.find(symbol);
        if (it == symbolSpecs_.end()) {
            // Unknown symbol: enforce safe default (1.0 min)
            std::printf("[CTraderFIX] WARNING: No spec for %s, using min=1.0\n",
                       symbol.c_str());
            return std::max(qty, 1.0);
        }
        
        const SymbolSpec& spec = it->second;
        
        // Enforce minimum
        if (qty < spec.minVolume) {
            std::printf("[CTraderFIX] Volume %.2f -> %.2f (min for %s)\n",
                       qty, spec.minVolume, symbol.c_str());
            qty = spec.minVolume;
        }
        
        // Enforce maximum
        if (qty > spec.maxVolume) {
            std::printf("[CTraderFIX] Volume %.2f -> %.2f (max for %s)\n",
                       qty, spec.maxVolume, symbol.c_str());
            qty = spec.maxVolume;
        }
        
        // Snap to step
        if (spec.volumeStep > 0.0) {
            qty = std::floor(qty / spec.volumeStep) * spec.volumeStep;
            if (qty < spec.minVolume) qty = spec.minVolume;
        }
    ...
    std::atomic<bool> quoteStarted_{false};  // Prevent double-start of QUOTE
    ...
    // v4.18.0: Per-symbol volume constraints from broker
    struct SymbolSpec {
        double minVolume  = 1.0;    // Minimum order size
        double maxVolume  = 100.0;  // Maximum order size
        double volumeStep = 1.0;    // Lot step (must be multiple of this)
    };
    std::map<std::string, SymbolSpec> symbolSpecs_;
    
    // Per-symbol tick tracking - v7.08
    mutable std::unordered_map<std::string, SymbolTickStats> symbolTickStats_;
    
    // v4.2.2: Order latency tracking
    struct PendingOrder {
        std::string symbol;
    ...
    std::mutex orderMtx_;
    std::unordered_map<std::string, PendingOrder> pendingOrders_;  // clOrdID -> PendingOrder
    
    CTraderTickCallback onTick_;
    CTraderExecCallback onExec_;
    CTraderStateCallback onState_;
    CTraderLatencyCallback onLatency_;
};
