...
public:
    ...
    void registerCrypto(const std::vector<std::string>& syms) {
        for (const auto& s : syms) {
            crypto_.insert(normalize(s));
        }
    }
    ...
    bool isCrypto(const std::string& s) const {
        return crypto_.count(normalize(s)) > 0;
    }
    ...
    // Any known tradeable symbol
    bool isKnown(const std::string& s) const {
        std::string n = normalize(s);
        return forex_.count(n) > 0 || metals_.count(n) > 0 || 
               indices_.count(n) > 0 || crypto_.count(n) > 0;
    }
    ...
    std::vector<std::string> allCrypto() const {
        return std::vector<std::string>(crypto_.begin(), crypto_.end());
    }
    ...
    size_t cryptoCount() const { return crypto_.size(); }
    size_t totalCount() const { return forex_.size() + metals_.size() + indices_.size() + crypto_.size(); }
    ...
    void dump() const {
        ...
        printf("[SYMBOL-REGISTRY] Crypto (%zu):", crypto_.size());
        for (const auto& s : crypto_) printf(" %s", s.c_str());
        printf("\n");
    }
...
private:
    ...
    std::unordered_set<std::string> crypto_;
};
