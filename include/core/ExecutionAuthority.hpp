#pragma once
// =============================================================================
// CHIMERA EXECUTION AUTHORITY - v4.8.0 - THE SINGLE CHOKE POINT
...
enum class SymbolTier : uint8_t {
    PRIMARY = 0, SECONDARY = 1, OPPORTUNISTIC = 2, CRYPTO = 3, BLOCKED = 4
};

inline SymbolTier classifySymbolTier(const char* symbol) {
    if (!symbol) return SymbolTier::BLOCKED;
    if (strcmp(symbol, "BTCUSDT") == 0 || strcmp(symbol, "ETHUSDT") == 0 || 
        strcmp(symbol, "SOLUSDT") == 0) return SymbolTier::CRYPTO;
...
public:
    ...
    // =========================================================================
    bool allow(const char* symbol, bool is_crypto, bool fix_connected, 
               bool expansion, bool intent_is_live, ExecBlockReason* out_reason = nullptr) {
        ...
        // 3. FIX CHECK (CFD only)
        if (!is_crypto && !fix_connected) {
    ...
    // Convenience: crypto always has expansion=true, no FIX needed
    bool allowCrypto(const char* symbol, bool intent_is_live, ExecBlockReason* out_reason = nullptr) {
        return allow(symbol, true, false, true, intent_is_live, out_reason);
    }
    
    // Convenience: CFD needs FIX connection check
    bool allowCFD(const char* symbol, bool fix_connected, bool expansion, 
                  bool intent_is_live, ExecBlockReason* out_reason = nullptr) {
        return allow(symbol, false, fix_connected, expansion, intent_is_live, out_reason);
    }
