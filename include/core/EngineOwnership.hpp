...
// v4.11.0: BINANCE removed
// =============================================================================
enum class EngineId : uint8_t {
    UNKNOWN = 0,
    // BINANCE = 1 - REMOVED v4.11.0
    CFD = 2,          // CFD engine (cTrader FIX/OpenAPI)
...
public:
    ...
    // =========================================================================
    // MICRO-LIVE MODE - DISABLES TIME-BASED OWNERSHIP
    // Use this when running E2/CFD-only deployment without IncomeEngine
    // =========================================================================
    void setMicroLiveMode(bool enabled) {
        micro_live_mode_ = enabled;
        if (enabled) {
            printf("[ENGINE-OWNERSHIP] MICRO-LIVE MODE ENABLED\n");
            printf("[ENGINE-OWNERSHIP] Time-based NAS100 ownership: DISABLED\n");
            printf("[ENGINE-OWNERSHIP] CFD engine can trade NAS100 at ANY time\n");
        } else {
            printf("[ENGINE-OWNERSHIP] MICRO-LIVE MODE DISABLED\n");
            printf("[ENGINE-OWNERSHIP] Time-based NAS100 ownership: ACTIVE\n");
        }
    }
    
    bool isMicroLiveMode() const noexcept {
        return micro_live_mode_;
    }
    ...
    [[nodiscard]] bool canTradeNAS100(EngineId engine) const noexcept {
        // MICRO-LIVE MODE: CFD engine can ALWAYS trade NAS100
        if (micro_live_mode_) {
            return (engine == EngineId::CFD || engine == EngineId::SHADOW);
        }
    ...
    [[nodiscard]] NAS100Owner getNAS100Owner() const noexcept {
        // MICRO-LIVE MODE: CFD always owns NAS100
        if (micro_live_mode_) {
    ...
    void printConfig() const {
        ...
        if (micro_live_mode_) {
            printf("  MICRO-LIVE MODE: ENABLED (CFD always owns NAS100)\n");
        } else {
...
private:
    EngineOwnership() {
        ...
        // v4.11.0: BINANCE ENGINE REMOVED
        
        // SHADOW ENGINE: Same as CFD
        allowed_[EngineId::SHADOW] = allowed_[EngineId::CFD];
        
        // Don't print here - let main configure first, then call printConfig()
    }
    
    EnforcementMode enforcement_mode_;
    IncomeWindowConfig income_cfg_;
    bool micro_live_mode_ = true;  // v4.10.3: HARDCODED for micro-live build
