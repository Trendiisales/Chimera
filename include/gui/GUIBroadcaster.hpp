...
//          - Removed gui_net_lat_* placeholder fields
...
struct GUIState {
    ...
    // THIS IS THE ONLY REAL LATENCY SOURCE - wired from BinanceOrderSender via main_dual.cpp
...
public:
    ...
    // v4.9.27: Added missing method
    void initSymbols() {
        // Initialize symbol tracking - placeholder for now
    }
    ...
    // This is called from main_dual.cpp with data from HotPathLatencyTracker in BinanceOrderSender
    ...
    void updateSymbolTick(const char* symbol, double bid, double ask, double network_latency = 0.2) {
        ...
        if (symbol_count_ < MAX_SYMBOLS) {
            ...
            if (strstr(symbol, "USDT")) symbols_[symbol_count_].asset_class = 0;  // Crypto
            else if (strstr(symbol, "USD") || strstr(symbol, "JPY") || strstr(symbol, "EUR") || strstr(symbol, "GBP") || strstr(symbol, "CHF") || strstr(symbol, "AUD") || strstr(symbol, "CAD") || strstr(symbol, "NZD")) symbols_[symbol_count_].asset_class = 1;  // Forex
            else if (strstr(symbol, "XAU") || strstr(symbol, "XAG")) symbols_[symbol_count_].asset_class = 2;  // Metals
            else if (strstr(symbol, "NAS") || strstr(symbol, "SPX") || strstr(symbol, "US30") || strstr(symbol, "UK100") || strstr(symbol, "GER40")) symbols_[symbol_count_].asset_class = 3;  // Indices
            else symbols_[symbol_count_].asset_class = 0;
...
private:
    ...
    std::string build_state_json() {
        ...
        // This is wired from BinanceOrderSender's HotPathLatencyTracker via main_dual.cpp
        {
            char lat_buf[512];
...
<p><a href="/" style="color:#0f0">Main Dashboard</a> | <a href="/regime_dashboard.html" style="color:#0f0">Regime Dashboard</a></p>
    ...
    // v4.9.26: REMOVED gui_net_lat_* placeholder fields
    // The hot_path_latency is now the ONLY latency source
    // network_latency in JSON now mirrors hot_path data for backward compatibility
};
