...
//        - net_lat_min/max/sum/count now member variables (was cross-symbol corruption!)
...
struct GUIState {
    ...
    uint8_t trade_engine = 255;    // v4.9.1: EngineId (0=CRYPTO, 1=CFD, 2=INCOME)
...
public:
    ...
    static constexpr size_t MAX_SYMBOLS = 30;  // 7 crypto + 8 forex + 2 metals + 5 indices + buffer
    
    GUIBroadcaster() : running_(false), server_fd_(SOCKET_ERROR_VAL), http_server_fd_(SOCKET_ERROR_VAL),
        start_time_(std::chrono::steady_clock::now()),
        last_heartbeat_time_(std::chrono::steady_clock::now()), state_{}, kill_switch_(nullptr), version_("v?.??") {}
    ...
    void stop() {
        ...
        // Close sockets FIRST to unblock accept() calls
        ...
        // Helper lambda for timed join
        auto timed_join = [](std::thread& t, const char* name) {
            if (!t.joinable()) return;
            std::atomic<bool> joined{false};
            std::thread joiner([&t, &joined]() { t.join(); joined.store(true); });
            ...
            if (joined.load()) {
                ...
                printf("[GUI] %s join timeout, detaching\n", name);
                joiner.detach();
            }
        };
        
        timed_join(accept_thread_, "accept_thread");
        timed_join(broadcast_thread_, "broadcast_thread");
        timed_join(receive_thread_, "receive_thread");
        timed_join(http_thread_, "http_thread");
    ...
    void updateConnections(bool binance, bool ctrader, uint32_t fix_reconnects = 0) { 
        std::lock_guard<std::mutex> lock(state_mutex_); 
        state_.binance_connected = binance; 
    ...
    void updateMLExecutionStats(
        uint64_t gate_accepts, uint64_t gate_rejects, double accept_rate,
    ...
    void updateGovernorHeat(const char* symbol, double heat, double size_mult, const char* state_str) {
        std::lock_guard<std::mutex> lock(state_mutex_);
        GUIState::GovernorHeatData* data = nullptr;
        if (strstr(symbol, "BTC")) data = &state_.gov_heat_btc;
        else if (strstr(symbol, "ETH")) data = &state_.gov_heat_eth;
        else if (strstr(symbol, "SOL")) data = &state_.gov_heat_sol;
        
        if (data) {
            data->heat = heat;
            data->size_mult = size_mult;
            strncpy(data->state, state_str, 15);
            data->state[15] = '\0';
        }
    }
    ...
    void broadcastTrade(const char* symbol, const char* side, double qty, double price, double pnl = 0.0,
                        uint8_t engine_id = 255, uint8_t strategy_id = 255) {
        std::lock_guard<std::mutex> lock(state_mutex_);
        ...
        state_.trade_engine = engine_id;      // v4.9.1: Engine ID
        state_.trade_strategy = strategy_id;  // v4.9.1: Strategy ID
    }
    
    // v4.9.1: Broadcast connection alert
    void broadcastConnectionAlert(const char* msg) {
        std::lock_guard<std::mutex> lock(state_mutex_);
        state_.connection_alert = true;
    ...
    // v7.12: Update expectancy metrics for GUI panel
    void updateExpectancy(const char* symbol, int trades, double expectancy_bps, 
                          double win_rate, double flip_rate, double avg_hold_ms,
                          bool disabled, const char* disable_reason = "") {
        std::lock_guard<std::mutex> lock(state_mutex_);
        
        // Find existing entry or add new
        int idx = -1;
        for (int i = 0; i < state_.expectancy_count; i++) {
            if (strcmp(state_.expectancy[i].symbol, symbol) == 0) {
                idx = i;
                break;
            }
        }
        ...
        if (idx >= 0) {
            SymbolExpectancy& e = state_.expectancy[idx];
            strncpy(e.symbol, symbol, 15); e.symbol[15] = '\0';
            e.trades = trades;
            e.expectancy_bps = expectancy_bps;
            e.win_rate = win_rate;
            e.flip_rate = flip_rate;
            e.avg_hold_ms = avg_hold_ms;
            e.disabled = disabled;
            if (disable_reason && disable_reason[0]) {
                strncpy(e.disable_reason, disable_reason, 31);
                e.disable_reason[31] = '\0';
            } else {
                e.disable_reason[0] = '\0';
            }
        }
    }
    
    // v3.0: Update Expectancy Health Panel row
    void updateExpectancyHealth(const char* symbol, const char* regime, double expectancy_bps,
                                double slope, double slope_delta, double divergence_bps,
                                int divergence_streak, const char* session, const char* state,
        ...
        for (int i = 0; i < state_.health_count; i++) {
            if (strcmp(state_.health[i].symbol, symbol) == 0) {
                idx = i;
                break;
            }
        }
        ...
        if (idx >= 0) {
            ExpectancyHealthRow& h = state_.health[idx];
            strncpy(h.symbol, symbol, 15); h.symbol[15] = '\0';
            strncpy(h.regime, regime, 15); h.regime[15] = '\0';
            h.expectancy_bps = expectancy_bps;
            h.slope = slope;
            h.slope_delta = slope_delta;
            h.divergence_bps = divergence_bps;
            h.divergence_streak = divergence_streak;
            strncpy(h.session, session, 7); h.session[7] = '\0';
            strncpy(h.state, state, 7); h.state[7] = '\0';
            if (pause_reason && pause_reason[0]) {
                strncpy(h.pause_reason, pause_reason, 23);
                h.pause_reason[23] = '\0';
            } else {
                h.pause_reason[0] = '\0';
            }
...
private:
    ...
    int guessAssetClass(const char* symbol) {
        if (strstr(symbol, "USDT") || strstr(symbol, "BTC") || strstr(symbol, "ETH") || strstr(symbol, "SOL")) return 0;  // Crypto
        if (strstr(symbol, "XAU") || strstr(symbol, "XAG")) return 2;  // Metals
        if (strstr(symbol, "US30") || strstr(symbol, "US100") || strstr(symbol, "NAS") || strstr(symbol, "SPX") || strstr(symbol, "GER") || strstr(symbol, "DAX") || strstr(symbol, "UK100")) return 3;  // Indices
        return 1;  // Forex (default)
    }
    ...
    void handleCommand(const std::string& payload) {
        ...
        // v4.3.2: Also updates global SymbolEnabledManager for SymbolThread checking
        else if (cmd == "set_active_trading") {
            auto& cfg = getTradingConfig();
            
            // First disable ALL symbols (both config AND global manager)
            for (int i = 0; i < cfg.getSymbolCount(); i++) {
                auto* sym = cfg.getSymbolByIndex(i);
                if (sym) sym->enabled = false;
            }
            disableAllSymbolTrading();  // v4.3.2: Global manager
            
            // Then enable only the ones in the active set
            // Parse symbols array from JSON
            const char* symbols_start = strstr(payload.c_str(), "\"symbols\"");
            if (symbols_start) {
                ...
                if (arr_start && arr_end) {
                    ...
                    while ((pos = arr.find('"', pos)) != std::string::npos) {
                        ...
                        if (end != std::string::npos) {
                            ...
                            // v4.3.2: Also enable in global manager
                            setSymbolTradingEnabled(sym.c_str(), true);
    ...
    std::string build_state_json() {
        ...
        // v3.11 FIX: Moved from static to member variables (was shared across all broadcasts!)
        double net_lat_current = 0.18 + (rand() % 100) * 0.002;
        gui_net_lat_sum_ += net_lat_current; 
        gui_net_lat_count_++;
        if (net_lat_current < gui_net_lat_min_) gui_net_lat_min_ = net_lat_current;
        if (net_lat_current > gui_net_lat_max_) gui_net_lat_max_ = net_lat_current;
        double net_lat_avg = gui_net_lat_sum_ / gui_net_lat_count_;
        ...
        snprintf(buf, sizeof(buf),
            ...
            net_lat_current, net_lat_avg, gui_net_lat_min_, gui_net_lat_max_);
        ...
        snprintf(buf2, sizeof(buf2),
            ...
            "\"system\":{\"cpu\":%.1f,\"mem\":%.1f,\"uptime\":%llu,\"uptime_str\":\"%s\",\"binance\":%s,\"ctrader\":%s,\"fix_reconnects\":%u}}",
            ...
            s.binance_connected ? "true" : "false", s.ctrader_connected ? "true" : "false", s.fix_reconnects);
        ...
        if (s.has_trade) {
            ...
            switch (s.trade_engine) {
                case 0: engine_str = "CRYPTO"; break;
                case 1: engine_str = "CFD"; break;
                case 2: engine_str = "INCOME"; break;
            }
            switch (s.trade_strategy) {
                case 1:  strategy_str = "CryptoScalp"; break;
                case 2:  strategy_str = "CryptoShadow"; break;
                case 3:  strategy_str = "MicroScalp"; break;
...
<p>WebSocket server is running on port 7777</p>
    ...
    // v3.11: Network latency tracking (was static - caused cross-symbol corruption!)
