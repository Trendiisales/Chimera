...
struct SuppressionEvent {
    ...
    char symbol[16] = {0};
    char venue[16] = {0};           // "CFD" only (v4.12.0)
...
inline SymbolClass getSymbolClass(const char* symbol) {
    // Metals and indices are CFD
    if (strstr(symbol, "XAU") || strstr(symbol, "XAG") || 
        strstr(symbol, "NAS") || strstr(symbol, "SPX") || strstr(symbol, "US30")) {
...
public:
    ...
    void recordCleanFill(const char* symbol, const char* venue, double latency_ms, double slippage_bps) {
        ...
        // Check if fill qualifies as "clean" - v4.12.0: CFD thresholds only
        double max_lat = (cls == SymbolClass::CFD) ? 80.0 : 100.0;  // FX slightly more lenient
        double max_slip = (cls == SymbolClass::CFD) ? 1.5 : 2.0;
