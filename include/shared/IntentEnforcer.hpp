...
//   - Integrated with TradeDecisionState for diagnostics
//
// USAGE:
...
private:
    ...
    size_t symbolIndex(const char* symbol) const {
        ...
        if (strcmp(symbol, "BTCUSDT") == 0) return 7;
        if (strcmp(symbol, "ETHUSDT") == 0) return 8;
        if (strcmp(symbol, "XAGUSD") == 0) return 9;
...
public:
    ...
    [[nodiscard]] static CheckResult check(
        ...
        // =====================================================================
        if (!EngineOwnership::instance().isAllowed(engine_id, symbol)) {
