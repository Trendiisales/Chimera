...
// BTCUSDT - Crypto, microstructure-driven, latency-critical
// v6.68: Relaxed latency for live demo - production should tighten
inline RiskParams BTCUSDT() {
    RiskParams p;
    p.base_risk = 0.0005;      // 0.05% - small by default
    p.alpha_vol = 1.5;         // Volatility crushes size quickly
    p.alpha_spread = 2.0;      // Spread widening is critical
    p.beta_liquidity = 0.8;    // Liquidity reduces, never increases
    p.alpha_latency = 0.5;     // RELAXED: Was 3.0, now tolerates more latency
    p.dd_exponent = 2.0;
    p.max_size = 0.005;        // 0.5% max
    p.min_R = 0.05;            // RELAXED: Was 0.1, now allows smaller R
    return p;
}

// XAUUSD - Gold CFD, session-sensitive, spread-critical
// v6.68: Relaxed for live demo
inline RiskParams XAUUSD() {
    RiskParams p;
    p.base_risk = 0.0010;      // 0.10%
    p.alpha_vol = 1.2;         // Less aggressive than BTC
    p.alpha_spread = 2.0;      // RELAXED: Was 2.5
    p.beta_liquidity = 1.0;    // More stable liquidity
    p.alpha_latency = 0.5;     // RELAXED: Was 1.5, now more tolerant
    p.dd_exponent = 2.2;
    p.max_size = 0.008;
    p.min_R = 0.05;            // RELAXED: Was 0.1
    return p;
}
...
inline RiskParams NAS100() {
    ...
    p.alpha_vol = 0.9;         // Indices tolerate vol better
    p.alpha_spread = 1.5;      // RELAXED: Was 1.8
    p.beta_liquidity = 1.1;
    p.alpha_latency = 0.5;     // RELAXED: Was 1.2
    p.dd_exponent = 1.8;
    p.max_size = 0.010;
    p.min_R = 0.05;            // RELAXED: Was 0.1
    return p;
...
inline RiskParams EURUSD() {
    ...
    p.base_risk = 0.0015;
    p.alpha_vol = 1.0;
    p.alpha_spread = 1.2;      // RELAXED: Was 1.5
    p.beta_liquidity = 1.2;
    p.alpha_latency = 0.3;     // RELAXED: Was 1.0
    p.dd_exponent = 2.0;
    p.max_size = 0.012;
    p.min_R = 0.05;            // RELAXED: Was 0.08
    return p;
}
...
// v6.68: ALL profiles relaxed for live demo trading
// =============================================================================
namespace AllProfiles {

// --- CRYPTO ---
inline RiskParams BTCUSDT() { return InstrumentProfiles::BTCUSDT(); }

inline RiskParams ETHUSDT() {
    RiskParams p;
    p.base_risk = 0.0008;
    p.alpha_vol = 1.3;
    p.alpha_spread = 1.8;
...
inline RiskParams SOLUSDT() {
    RiskParams p;
    p.base_risk = 0.0006;
    p.alpha_vol = 1.8;        // SOL is more volatile
    p.alpha_spread = 2.2;
    p.beta_liquidity = 0.7;
    p.alpha_latency = 0.5;
    p.dd_exponent = 2.0;
    p.max_size = 0.004;
    p.min_R = 0.05;
...
inline RiskParams getProfile(const char* symbol) {
    if (strcmp(symbol, "BTCUSDT") == 0) return BTCUSDT();
    if (strcmp(symbol, "ETHUSDT") == 0) return ETHUSDT();
    if (strcmp(symbol, "SOLUSDT") == 0) return SOLUSDT();
...
inline double getSessionWeight(int utc_hour, const char* instrument) {
    // BTCUSDT - 24/7 but liquidity varies
    if (strcmp(instrument, "BTCUSDT") == 0) {
        if (utc_hour >= 7 && utc_hour <= 10) return 1.0;   // London
        if (utc_hour >= 13 && utc_hour <= 16) return 0.95; // NY
        if (utc_hour >= 21 || utc_hour <= 2) return 0.9;   // Asia
        return 0.8;
    }
