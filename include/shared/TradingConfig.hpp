...
struct AssetClassConfig {
    char name[16] = {0};
    int asset_class = 0;  // 0=crypto, 1=forex, 2=metals, 3=indices
...
private:
    ...
    double daily_loss_limit_ = -500.0;   // NZD
    ...
    void initDefaults() {
        ...
        // 0 = Crypto
        asset_classes_[0].setName("Crypto");
        asset_classes_[0].asset_class = 0;
        asset_classes_[0].default_size = 0.0005;
        asset_classes_[0].default_max_pos = 0.001;
        asset_classes_[0].default_sl_bps = 25;
        asset_classes_[0].default_tp_bps = 45;
        asset_classes_[0].default_max_spread_bps = 4;
        asset_classes_[0].default_vpin = 0.60;
        asset_classes_[0].default_ofi = 0.55;
        asset_classes_[0].default_cooldown_ms = 250;
        ...
        // Initialize symbols - ONLY core symbols enabled by default
        // v6.99: Fixed - disabled symbols must actually be disabled!
        addSymbol("BTCUSDT", 0, false);   // v7.06: ALL disabled by default - dashboard enables
        addSymbol("ETHUSDT", 0, false);   
        addSymbol("SOLUSDT", 0, false);   
    ...
    void loadConservative() {
        ...
        // Crypto - very conservative
        asset_classes_[0].default_size = 0.0003;
        asset_classes_[0].default_sl_bps = 15;
        asset_classes_[0].default_tp_bps = 30;
        asset_classes_[0].default_max_spread_bps = 3;
        asset_classes_[0].default_vpin = 0.55;
        asset_classes_[0].default_cooldown_ms = 500;
    ...
    void loadAggressive() {
        ...
        // Crypto - aggressive
        asset_classes_[0].default_size = 0.001;
        asset_classes_[0].default_sl_bps = 35;
        asset_classes_[0].default_tp_bps = 60;
        asset_classes_[0].default_max_spread_bps = 6;
        asset_classes_[0].default_vpin = 0.70;
        asset_classes_[0].default_cooldown_ms = 150;
    ...
    int getAssetClassForSymbol(const char* symbol) const {
        if (strstr(symbol, "USDT") || strstr(symbol, "BTC") || 
            strstr(symbol, "ETH") || strstr(symbol, "SOL")) return 0;
        if (strstr(symbol, "XAU") || strstr(symbol, "XAG")) return 2;
        if (strstr(symbol, "US30") || strstr(symbol, "NAS") || 
            strstr(symbol, "SPX") || strstr(symbol, "DAX")) return 3;
