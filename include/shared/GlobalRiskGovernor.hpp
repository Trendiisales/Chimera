...
enum class AggressionState : uint8_t {
    ...
    REDUCED,        // Income scratched → CFD reduced
...
public:
    ...
    // v4.11.0: crypto kill-on-first-loss removed
    // =========================================================================
    ...
    void printStatus() const {
        ...
        printf("  Trades: Income=%d/%d, CFD=%d/%d\n",
    ...
    // For GUI/dashboard JSON (v4.11.0: crypto fields removed)
    void toJSON(char* buf, size_t bufsize) const {
        snprintf(buf, bufsize,
            "{\"daily_pnl\":%.2f,\"daily_limit\":%.0f,\"dd_used\":%.3f,\"throttle\":%.3f,"
            "\"aggression\":\"%s\",\"income_trades\":%d,\"cfd_trades\":%d,"
            "\"consecutive_losses\":%d,\"shutdown\":\"%s\"}",
            daily_loss_ ? daily_loss_->pnl() : 0.0,
            daily_loss_ ? daily_loss_->limit() : 0.0,
            drawdownUsed(),
            throttleFactor(),
            aggression_str(aggression_state_.load()),
            income_trades_, cfd_trades_,
            consecutive_losses_,
            shutdown_reason_str(shutdown_reason_));
    }
...
private:
    ...
    // v4.11.0: crypto removed from getTradeCount
    ...
    // Trade counts (per day) - v4.11.0: crypto removed
