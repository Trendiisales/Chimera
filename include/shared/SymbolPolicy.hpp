// =============================================================================
// SymbolPolicy.hpp - v4.7.0 - PRE-FIX SYMBOL RULES
// =============================================================================
// PURPOSE: Defines which symbols are allowed and under what conditions
//
// UNTIL FIX IS LIVE, CHIMERA MUST NOT BE DEMOCRATIC.
// Pick ONE symbol per venue type and stick to it.
//
// PRE-FIX ALLOWED SYMBOLS:
//   CRYPTO: BTCUSDT (firm quotes, cheap probes, clear structure)
//   CFD:    XAUUSD NY session only (if FIX connected)
//
// PRE-FIX DISABLED (too expensive without FIX):
//   - NAS100 (spread flickers, probes are spread tax)
//   - US30, SPX500 (same issues)
//   - All other CFD indices
//
// OWNERSHIP: Jo
...
inline const std::unordered_map<std::string, SymbolPolicy>& getPreFIXPolicy() {
    static const std::unordered_map<std::string, SymbolPolicy> policy = {
        ...
        {"BTCUSDT", {
            true,                       // allowed
            SymbolType::CRYPTO,         // type
            SessionWindow::ANY,         // session (24/7)
            0.5,                        // min_spread_bps (reject < 0.5 - anomaly)
            8.0,                        // max_spread_bps
            true,                       // probes_allowed (cheap on crypto)
            true,                       // shadow_allowed
            1                           // tier (primary)
        }},
        {"ETHUSDT", {
