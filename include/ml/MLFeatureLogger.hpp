...
public:
    ...
    inline bool logEntry(
        ...
        uint8_t engine_id = static_cast<uint8_t>(EngineId::UNKNOWN)
    ) noexcept {
        MLFeatureRecord rec;
        rec.timestamp_ns = ts;
        ...
        rec.engine_id = engine_id;  // v4.9.1
    ...
    inline bool logClose(
        ...
        float ofi, float vpin, float conviction, float spread_bps,
        uint16_t min_open, int8_t side, uint8_t strat_id,
        uint8_t engine_id,  // v4.9.1: engine_id before outcome params
...
public:
    static bool exportToCSV(const char* binary_path, const char* csv_path) noexcept {
        ...
        // Header (matches simplified 64-byte record) - v4.9.1: Added engine_id
        std::fprintf(fout, "timestamp_ns,symbol_id,state,intent,regime,side,"
                           "ofi,vpin,spread_bps,conviction,"
                           "min_open,strategy_id,engine_id,realized_R,mfe_R,mae_R,hold_ms\n");
