...
struct alignas(64) MLFeatureRecord {
    ...
    uint8_t  padding1[4];           // Alignment
    
    // ── State Classification (4 bytes) ──
    MLMarketState state;            // Current market state
    MLTradeIntent intent;           // Proposed trade intent
    MLRegime regime;                // Volatility regime
    uint8_t  padding2;              // Alignment
    
    // ── Microstructure Features (24 bytes) ──
    float atr_multiple;             // ATR / baseline_ATR
    float volume_z;                 // (volume - mean) / std
    float range_z;                  // (range - mean) / std
    float distance_vwap;            // (mid - VWAP) / ATR
    float ofi;                      // Order flow imbalance [-1, +1]
    float vpin;                     // Toxicity [0, 1]
    
    // ── Setup Features (12 bytes) ──
    float conviction_score;         // [0, 10] from MarketStateClassifier
    float spread_bps;               // Current spread in basis points
    float trend_strength;           // [0, 1] momentum consistency
    
    // ── Context Features (4 bytes) ──
    uint16_t minutes_from_open;     // Minutes since market open
    int8_t   side;                  // +1 BUY, -1 SELL, 0 none
    uint8_t  strategy_id;           // Which bucket strategy triggered
    
    // ── Outcome Labels (12 bytes) - Filled on trade close ──
