// =============================================================================
// VwapDefenseProfile.hpp - v4.9.0 - VWAP DEFENSE / RECLAIM ENGINE
// =============================================================================
...
// =============================================================================
// VWAP DEFENSE SYMBOL CONFIG
// =============================================================================
struct VDSymbolConfig {
    bool enabled = false;
    double vwapProximityPct = 0.0015;    // Price must be within this % of VWAP
    uint64_t reclaimHoldNs = 300'000'000ULL;  // 300ms hold for reclaim
    uint64_t failWindowNs = 400'000'000ULL;   // 400ms window for fail fade
    double supportiveImbalance = 0.40;    // Imbalance threshold for support
    double collapseImbalance = 0.25;      // Imbalance collapse threshold
    double edgeDecayExit = 0.45;          // Exit if edge decays > 45%
    uint64_t maxHoldNs = 6'000'000'000ULL;   // 6 second time cap
    uint64_t cooldownNs = 300'000'000ULL;    // 300ms cooldown
};
...
inline VDSymbolConfig getVDConfig(const char* symbol) {
    ...
    if (strcmp(symbol, "NAS100") == 0) {
        ...
        cfg.failWindowNs = 400'000'000ULL;
        cfg.supportiveImbalance = 0.42;
        cfg.collapseImbalance = 0.25;
        cfg.edgeDecayExit = 0.45;
        cfg.maxHoldNs = 6'000'000'000ULL;
        cfg.cooldownNs = 300'000'000ULL;
    }
    else if (strcmp(symbol, "US30") == 0) {
        cfg.enabled = true;
        cfg.vwapProximityPct = 0.0010;
        cfg.reclaimHoldNs = 350'000'000ULL;
        cfg.failWindowNs = 450'000'000ULL;
        cfg.supportiveImbalance = 0.38;
        cfg.collapseImbalance = 0.22;
        cfg.edgeDecayExit = 0.45;
        cfg.maxHoldNs = 7'000'000'000ULL;
        cfg.cooldownNs = 350'000'000ULL;
    }
    else if (strcmp(symbol, "SPX500") == 0) {
        cfg.enabled = true;
        cfg.vwapProximityPct = 0.0015;
        cfg.reclaimHoldNs = 280'000'000ULL;
        cfg.failWindowNs = 380'000'000ULL;
        cfg.supportiveImbalance = 0.45;
        cfg.collapseImbalance = 0.28;
        cfg.edgeDecayExit = 0.42;
        cfg.maxHoldNs = 5'000'000'000ULL;
        cfg.cooldownNs = 280'000'000ULL;
    }
    else if (strcmp(symbol, "XAUUSD") == 0) {
        cfg.enabled = true;
        cfg.vwapProximityPct = 0.0018;
        cfg.reclaimHoldNs = 350'000'000ULL;
        cfg.failWindowNs = 500'000'000ULL;
        cfg.supportiveImbalance = 0.35;
        cfg.collapseImbalance = 0.20;
        cfg.edgeDecayExit = 0.48;
...
// =============================================================================
// SESSION POLICY FOR VWAP DEFENSE
// =============================================================================
...
inline VDSessionPolicy getVDSessionPolicy(const std::string& session) {
    ...
    if (session == "NY_OPEN" || session == "NY" || session == "NY_MID") {
        ...
        // Disabled for Asia - VWAP defense less reliable
...
struct VwapTestState {
    bool testing = false;
    bool wasAboveVwap = false;        // Position before test
    double testStartPrice = 0.0;
    uint64_t testStartNs = 0;
    uint64_t crossedVwapNs = 0;       // When price crossed VWAP
    double crossedImbalance = 0.0;    // Imbalance at cross
...
private:
    ...
    // Real sizing function - VWAP defense uses tighter risk (0.05-0.10%)
    double calculateQty(double riskPct, const VDMarketSnapshot& snap) {
        ...
        // Conservative sizing for VWAP defense trades
        double riskAmount = 10000.0 * riskPct;
        double stopDistPts = spreadPts * 2.5;  // Tighter stops for VWAP defense
        ...
        return std::clamp(qty, 0.01, 5.0);  // Smaller max for VWAP trades
    }
};
