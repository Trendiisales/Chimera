...
// =============================================================================
// GET ROUTING ENTRY
// v4.11.0: crypto symbols removed
// =============================================================================
inline const VenueRoutingEntry& GetRoutingEntry(const char* symbol) {
    ...
    if (strstr(symbol, "NAS100") || strstr(symbol, "US100") || strstr(symbol, "US30")) return NAS100_ROUTING_ENTRY;
...
inline OrderType ChooseOrderType(
    ...
    (void)symbol;  // v4.11.0: All CFD, no symbol-specific routing
    ...
    if (aggressive) {
        if (net_edge >= EDGE_STRONG && latency_ms <= LATENCY_FAST_MS) {
            return OrderType::IOC;  // v4.11.0: CFD uses IOC
        }
