...
// ─────────────────────────────────────────────────────────────────────────────
// v4.11.0: is_crypto removed - CFD only
inline SymbolExecutionProfile buildProfile(
    ...
    double ack_p95_ms,
    double reject_rate,
    bool taker_only = false  // v4.11.0: explicit param instead of is_crypto
) {
    ...
    p.latency_penalty_bps_per_ms = 0.25;  // CFD default
    p.reject_rate = reject_rate;
    
    p.taker_only = taker_only;
    p.maker_primary = !taker_only;
...
inline SymbolExecutionProfile getDefaultProfile(const char* symbol) {
    ...
    if (strcmp(symbol, "XAUUSD") == 0) {
        return buildProfile("XAUUSD", 3.0, 0.8, 5.0, 0.03);
    }
    // XAGUSD
    if (strcmp(symbol, "XAGUSD") == 0) {
        return buildProfile("XAGUSD", 3.5, 1.2, 5.5, 0.04);
    }
    // NAS100
    if (strcmp(symbol, "NAS100") == 0) {
        return buildProfile("NAS100", 2.5, 0.5, 8.0, 0.02);
    }
    // US30
    if (strcmp(symbol, "US30") == 0) {
        return buildProfile("US30", 2.5, 0.6, 8.0, 0.02);
    }
    
    // Default CFD
    return buildProfile(symbol, 4.0, 1.0, 3.0, 0.05);
}
