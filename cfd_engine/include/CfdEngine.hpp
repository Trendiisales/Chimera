...
// v3.11: XAGUSD params fixed, STATIC VARIABLE AUDIT COMPLETE
...
// v3.5: Fixed XAUUSD/XAGUSD params for BlackBull
...
// v4.9.35: Compile-time switch between FIX and Open API
#ifdef USE_OPENAPI
...
#include "../include/risk/ExpectancyAuthority.hpp"
...
// v4.12.1: COMPILE FIX - Removed heavy headers that caused 4x compile time:
// - engines/IndexImpulseEngine.hpp (826 lines) - include in main_microlive.cpp only
// - engines/GoldEngine_v5_2.hpp - include in main_microlive.cpp only  
// - portfolio/PortfolioModeController.hpp (491 lines) - include in main_microlive.cpp only
// Only include lightweight integration headers here:
#include "quality/MarketQualityCuts.hpp"
#include "integration/CfdEngineIntegration.hpp"

// Forward declarations for engine types (actual headers in main_microlive.cpp)
namespace Chimera {
    class IndexImpulseEngine;
    class GoldEngineV5;
    namespace Portfolio { class PortfolioModeController; }
}
...
// v4.9.35: Conditional types based on protocol
#ifdef USE_OPENAPI
using Chimera::CTraderOpenAPIClient;
using Chimera::OpenAPIConfig;
#else
...
struct CfdEngineStats {
    ...
    // v4.9.34: FIX RTT latency tracking (order send → ACK)
    static constexpr size_t RTT_BUFFER_SIZE = 256;
    std::array<double, RTT_BUFFER_SIZE> rtt_samples{};
    std::atomic<size_t> rtt_count{0};
    std::atomic<double> rtt_min_ms{999999.0};
    std::atomic<double> rtt_max_ms{0.0};
    std::atomic<double> rtt_sum_ms{0.0};
    
    void recordRtt(double rtt_ms) {
        size_t idx = rtt_count.fetch_add(1, std::memory_order_relaxed) % RTT_BUFFER_SIZE;
        rtt_samples[idx] = rtt_ms;
        rtt_sum_ms.fetch_add(rtt_ms, std::memory_order_relaxed);
        
        // Update min/max atomically (CAS loop)
        double old_min = rtt_min_ms.load(std::memory_order_relaxed);
        while (rtt_ms < old_min && !rtt_min_ms.compare_exchange_weak(old_min, rtt_ms)) {}
        
        double old_max = rtt_max_ms.load(std::memory_order_relaxed);
        while (rtt_ms > old_max && !rtt_max_ms.compare_exchange_weak(old_max, rtt_ms)) {}
    }
    
    double rttAvgMs() const {
        size_t count = rtt_count.load(std::memory_order_relaxed);
        if (count == 0) return 0.0;
        return rtt_sum_ms.load(std::memory_order_relaxed) / count;
    }
    
    double rttMinMs() const {
        double m = rtt_min_ms.load(std::memory_order_relaxed);
        return m > 100000.0 ? 0.0 : m;
    }
    
    double rttMaxMs() const { return rtt_max_ms.load(std::memory_order_relaxed); }
    
    size_t rttSamples() const { return rtt_count.load(std::memory_order_relaxed); }
    
    // Compute p50 and p99 from buffer (not thread-safe, call from single thread)
    std::pair<double, double> rttPercentiles() const {
        size_t count = rtt_count.load(std::memory_order_relaxed);
        if (count == 0) return {0.0, 0.0};
        
        size_t n = std::min(count, RTT_BUFFER_SIZE);
        std::vector<double> sorted(rtt_samples.begin(), rtt_samples.begin() + n);
...
public:
        ...
        : running_(false), connected_(false), kill_switch_(nullptr), execEngine_(centralMicro_)
    {
        ...
        std::cout << "\n[CFD-ENGINE] v4.12.0 INTENT-BASED EXECUTION POLICY:\n"
                  << "  ════════════════════════════════════════════════════════════\n"
                  << "  🔒 THE PRIME DIRECTIVE: No execution unless INTENT == LIVE\n"
                  << "  ════════════════════════════════════════════════════════════\n"
                  << "  PRE-FIX ALLOWED: XAUUSD (NY expansion only)\n"
    ...
    // v4.9.35: Config setter - works with FIXConfig or OpenAPIConfig
#ifdef USE_OPENAPI
    void setConfig(const OpenAPIConfig& cfg) { clientConfig_ = cfg; }
    void setFIXConfig(const OpenAPIConfig& cfg) { clientConfig_ = cfg; }  // Legacy alias
#else
    void setConfig(const FIXConfig& cfg) { clientConfig_ = cfg; }
    void setFIXConfig(const FIXConfig& cfg) { clientConfig_ = cfg; }  // Legacy alias
#endif
    ...
    void setIntentLive(bool live) noexcept { client_.setIntentLive(live); }
    bool isIntentLive() const noexcept { return client_.isIntentLive(); }
    void setNYExpansion(bool active) noexcept { client_.setNYExpansion(active); }
    
    // v7.08: Print per-symbol FIX tick stats
    void printSymbolTickStats() const {
        client_.printSymbolTickStats();
    }
    ...
    bool start() {
        ...
        // v4.10.0: Initialize new engine integration
        Chimera::getCfdEngineIntegration().init(100000.0);  // Default starting equity
        ...
        std::cout << "[CfdEngine] v4.10.0: Index Impulse + Gold Campaign engines ACTIVE" << std::endl;
        return true;
    }
    ...
    inline void processTick(const TickFull& tick) {
        ...
        // =====================================================================
        // v4.10.0: NEW ENGINE PROCESSING (Index Impulse, Gold Campaign)
        // These engines take priority for their respective symbol classes
        // =====================================================================
        ...
        auto& engine_integration = Chimera::getCfdEngineIntegration();
        bool new_engine_handled = false;
        Chimera::IntegrationOutput new_engine_out;
        
        // Route to new engines for indices and gold
        Chimera::SymbolClass sym_class = Chimera::getSymbolClass(tick.symbol);
        if (sym_class == Chimera::SymbolClass::CFD) {
            new_engine_out = engine_integration.onTick(
                tick.symbol, tick.bid, tick.ask, 
                (tick.bid_size + tick.ask_size) / 2.0,  // volume proxy
                start_ns
            );
            
            if (new_engine_out.should_trade && connected_ && symbol_enabled) {
                // New engine wants to trade
                DBG_LOGF("NEW_ENGINE_SIGNAL %s: engine=%s dir=%d size=%.4f reason=%s",
                         tick.symbol, new_engine_out.engine, new_engine_out.direction,
                         new_engine_out.size, new_engine_out.reason);
                
                if (riskGuard_.checkOrder(new_engine_out.size, new_engine_out.direction)) {
                    double trade_pnl = new_engine_out.is_exit ? new_engine_out.realized_pnl : 0.0;
                    submitOrder(Chimera::EngineId::CFD, tick.symbol, new_engine_out.direction, 
                               new_engine_out.size, mid, trade_pnl);
                    stats_.consensus_trades.fetch_add(1, std::memory_order_relaxed);
                    
                    if (new_engine_out.is_exit && pnlCallback_) {
                        pnlCallback_(tick.symbol, new_engine_out.realized_pnl, true);
                        // Update portfolio controller with result
                        double risk_dollars = new_engine_out.size * mid * 0.005;  // Approx 0.5% risk
                        engine_integration.onTradeResult(tick.symbol, new_engine_out.direction, new_engine_out.realized_pnl, risk_dollars);
                    }
                    
                    std::cout << "[" << new_engine_out.engine << "-TRADE] " << tick.symbol
                              << (new_engine_out.direction > 0 ? " BUY " : " SELL ")
                              << new_engine_out.size << " @ " << mid
                              << " reason=" << new_engine_out.reason;
                    if (new_engine_out.is_exit) {
                        std::cout << " PnL=" << trade_pnl;
                    }
                    std::cout << "\n";
                    
                    new_engine_handled = true;
                } else {
        ...
        // =====================================================================
        // EXECUTE TRADE (LIVE) - Pure Scalper fallback for FX
        // =====================================================================
        
        // v7.03 DEBUG: Log trade signals with enable status
        if (scalp.shouldTrade() && !new_engine_handled) {
        ...
        if (scalp.shouldTrade() && connected_ && symbol_enabled && !new_engine_handled) {
    ...
    bool isConnected() const { 
        // v4.9.12: Check actual FIX session state, not stale flag
        return client_.isConnected(); 
    }
    
    // v4.9.34: FIX Session RTT (co-located latency from TestRequest/Heartbeat)
    double fixRttLastMs() const noexcept { return client_.fixRttLastMs(); }
    double fixRttMinMs() const noexcept { return client_.fixRttMinMs(); }
    double fixRttMaxMs() const noexcept { return client_.fixRttMaxMs(); }
    double fixRttAvgMs() const noexcept { return client_.fixRttAvgMs(); }
    size_t fixRttSamples() const noexcept { return client_.fixRttSamples(); }
    
    // =========================================================================
    // v4.9.9: PUBLIC ORDER SUBMISSION FOR EXTERNAL ENGINES (MicroScalp)
    // =========================================================================
    // This allows MetalMicroScalp and IndexMicroScalp to submit orders through
    // CfdEngine's FIX connection without duplicating connection logic.
    //
    // NOTE: These orders bypass CfdEngine's internal trading logic (PureScalper)
    // and go directly to the FIX client. Risk checks are the caller's responsibility.
    // =========================================================================
    bool sendExternalOrder(const char* symbol, int8_t side, double qty) {
        if (!connected_) {
            printf("[CFD-EXT] ORDER REJECTED: Not connected to FIX\n");
            return false;
        }
        if (kill_switch_ && kill_switch_->isCfdKilled()) {
            printf("[CFD-EXT] ORDER REJECTED: Kill switch active\n");
            return false;
        }
        
        char fix_side = (side > 0) ? '1' : '2';  // FIX: 1=Buy, 2=Sell
        bool sent = client_.sendMarketOrder(symbol, fix_side, qty);
        
        if (sent) {
            printf("[CFD-EXT] Order sent: %s %s %.4f\n", 
                   symbol, side > 0 ? "BUY" : "SELL", qty);
        } else {
            printf("[CFD-EXT] Order FAILED: %s %s %.4f\n",
                   symbol, side > 0 ? "BUY" : "SELL", qty);
        }
...
private:
    void engineLoop() {
        ...
        std::cout << "[CfdEngine] Connecting to OpenAPI: " << clientConfig_.host << ":" << clientConfig_.port << std::endl;
        DBG_LOG("CfdEngine loop started");
        DBG_LOGF("OpenAPI Config: host=%s port=%d", 
                 clientConfig_.host.c_str(), clientConfig_.port);
        
        // Setup FIX callbacks
        client_.setConfig(clientConfig_);
        client_.setOnTick([this](const CTraderTick& t) { onFIXMarketData(t); });
        client_.setOnExec([this](const CTraderExecReport& r) { onFIXExecution(r); });
        client_.setOnLatency([this](const std::string& symbol, double rtt_ms, double slippage_bps) {
        ...
        while (running_) {
            ...
            if (!client_.isConnected()) {
                connected_.store(false);  // Keep flag in sync
                reconnectAttempts++;
                ...
                DBG_LOG("Calling client_.connect()...");
                if (client_.connect()) {
                    connected_ = true;
                    reconnectAttempts = 0;
                    ...
                    client_.requestSecurityList();
                    int waitCount = 0;
                    while (!client_.isSecurityListReady() && waitCount < 300 && running_) {
                        std::this_thread::sleep_for(std::chrono::milliseconds(100));
                        waitCount++;
                    }
                    
                    if (client_.isSecurityListReady()) {
                        DBG_LOG("Security list ready, subscribing symbols");
                        for (const auto& sym : forex_symbols_) client_.subscribeMarketData(sym);
                        for (const auto& sym : metals_symbols_) client_.subscribeMarketData(sym);
                        for (const auto& sym : indices_symbols_) client_.subscribeMarketData(sym);
                        DBG_LOG("Subscribed to all symbols");
                    } else {
                        DBG_ERROR("Security list timeout!");
                    }
                } else {
                    DBG_ERROR("client_.connect() returned false");
                    DBG_CONN("FIX", false);
    ...
    void onFIXLatency(const std::string& symbol, double rtt_ms, double slippage_bps) {
        // v4.9.34: Record RTT to stats for GUI display
        stats_.recordRtt(rtt_ms);
    ...
    CfdSymbolMeta getSymbolMeta(const char* symbol) const {
        ...
        // XAUUSD / Gold - BlackBull 0.01 lot minimum
        if (sym == "XAUUSD" || sym == "GOLD") {
            meta.min_trade_size = 0.01;   // v4.9.9: Fixed - BlackBull allows 0.01
            meta.contract_size = 100.0;  // 100 oz per lot
            meta.tick_size = 0.01;
            meta.is_index = false;
        }
        // XAGUSD / Silver - BlackBull 0.01 lot minimum
    ...
    inline void processIntents() {
        ...
        while (intentQueue_.try_pop(intent)) {
            ...
            if (connected_) {
                ...
                bool sent = client_.sendMarketOrder(intent.symbol, fixSide, intent.qty);
...
#ifdef USE_OPENAPI
    OpenAPIConfig clientConfig_;
    mutable CTraderOpenAPIClient client_;
#else
    FIXConfig clientConfig_;
    mutable CTraderFIXClient client_;
#endif
