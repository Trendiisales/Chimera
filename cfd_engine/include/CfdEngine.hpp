...
public:
    ...
    bool isConnected() const { 
        ...
        return fixClient_.isConnected(); 
    }
    
    // v4.9.34: FIX Session RTT (co-located latency from TestRequest/Heartbeat)
    double fixRttLastMs() const noexcept { return fixClient_.fixRttLastMs(); }
    double fixRttMinMs() const noexcept { return fixClient_.fixRttMinMs(); }
    double fixRttMaxMs() const noexcept { return fixClient_.fixRttMaxMs(); }
    double fixRttAvgMs() const noexcept { return fixClient_.fixRttAvgMs(); }
    size_t fixRttSamples() const noexcept { return fixClient_.fixRttSamples(); }
    ...
    bool sendExternalOrder(const char* symbol, int8_t side, double qty) {
        ...
        bool sent = fixClient_.sendMarketOrder(symbol, fix_side, qty);
