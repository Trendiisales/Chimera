...
public:
        ...
        : running_(false), connected_(false), kill_switch_(nullptr), execEngine_(centralMicro_)
    {
        ...
        shadow_state_["XAUUSD"].tier = 2;   // Gold - primary
        shadow_state_["NAS100"].tier = 2;   // Nasdaq
        shadow_state_["US100"].tier = 2;    // Nasdaq alias
        shadow_state_["SPX500"].tier = 2;   // S&P 500
        ...
        std::cout << "[CFD-SHADOW] v4.2.2 Tier assignments:\n"
                  << "  TIER 2 (ACTIVE): XAUUSD, NAS100, SPX500, EURUSD, GBPUSD\n"
                  << "  TIER 3 (SENSOR): XAGUSD, USDJPY, AUDUSD, US30, GER40\n"
    ...
    inline void processTick(const TickFull& tick) {
        ...
        // Only process shadow for TIER 2 and TIER 3 symbols
        if (shadow.tier >= 2 && shadow.tier <= 3) {
            ...
            if (shadow.position_open) {
                ...
                if (should_exit) {
                    ...
                    logShadowTradeCSV(tick.symbol, shadow.side > 0 ? "LONG" : "SHORT",
...
private:
    ...
    void initShadowCSV() {
        ...
        if (shadow_csv_.is_open()) {
            shadow_csv_ << "timestamp,symbol,side,entry_price,exit_price,pnl_bps,hold_ms,exit_reason,trade_num\n";
            shadow_csv_.flush();
            csv_initialized_ = true;
            std::cout << "[CSV] Trade logging initialized: cfd_shadow_trades.csv\n";
        }
    }
    
    void logShadowTradeCSV(const char* symbol, const char* side, double entry, double exit,
                           double pnl_bps, uint64_t hold_ms, const char* exit_reason, uint64_t trade_num) {
