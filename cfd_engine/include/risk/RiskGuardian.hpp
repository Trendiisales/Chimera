...
// =============================================================================
// v6.46: Fixed NaN/Inf handling for uninitialized signals
// =============================================================================
#pragma once
...
struct alignas(CACHE_LINE_SIZE) RiskLimits {
    ...
    // Warmup period - skip checks for first N ticks while signals initialize
    uint64_t warmupTicks = 10;
};
...
struct alignas(CACHE_LINE_SIZE) RiskState {
    ...
    std::atomic<uint64_t> tickCount{0};  // For warmup tracking
...
public:
    ...
    // ==========================================================================
    // Helper: Check if a value is valid (not NaN, not Inf, not extreme)
    // ==========================================================================
    static inline bool isValidSignal(double v) {
        return std::isfinite(v) && v >= -1e10 && v <= 1e10;
    }
    ...
    inline bool checkTick(const UnifiedTick& /*t*/, const MicrostructureSignals& sig) {
        // Track tick count for warmup
        uint64_t ticks = state_.tickCount.fetch_add(1, std::memory_order_relaxed);
        ...
        // =====================================================================
        // WARMUP PERIOD: Skip signal checks until we have enough data
        // This prevents false blocks from uninitialized/NaN signals
        // =====================================================================
        if (ticks < limits_.warmupTicks) {
            return true;  // Allow trading during warmup
        }
        
        // =====================================================================
        // SIGNAL VALIDITY CHECKS
        // Skip checks for invalid signals (NaN, Inf, uninitialized)
        // This handles FIX data which doesn't have volume info
        // =====================================================================
        
        // Quick volatility check (only if valid)
        if (isValidSignal(sig.realizedVolatility)) {
        ...
        // Quick toxicity check (only if valid)
        if (isValidSignal(sig.toxicity) && isValidSignal(sig.vpin)) {
        ...
        // Quick spread check (only if valid)
        // Note: spreadBps can be Inf/NaN if VWAP is 0 (no volume data)
        if (isValidSignal(sig.spreadBps)) {
        ...
        // If spreadBps is invalid (NaN/Inf), we ALLOW the tick through
        // This handles FIX quote data which has no volume info
    ...
    inline uint64_t getTickCount() const {
        return state_.tickCount.load(std::memory_order_relaxed);
    }
