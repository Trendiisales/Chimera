...
public:
    ...
    // v4.2.2: Returns clOrdID on success, empty string on failure (for latency tracking)
...
private:
    ...
    void onRawMessage(const std::string& raw) {
        ...
        // v3.6: Only log important messages, NOT market data (W=MarketDataSnapshot)
        // This was causing 82% system CPU from stdout I/O
        char msgType = msg.getMsgType();
        if (msgType != 'W' && msgType != '0' && msgType != '1') {  // Skip quotes, heartbeats, test req
            printMessage("RX", raw);
        }
        ...
        // Handle by message type (msgType already declared above for logging filter)
        switch (msgType) {
