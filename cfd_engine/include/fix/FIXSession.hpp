...
public:
    ...
    // v4.7.0: sendNewOrder with ExecutionAuthority as FIRST GATE
    std::string sendNewOrder(const std::string& symbol, char side, double qty, 
        ...
        bool fix_connected = (state_.load() == FIXSessionState::LOGGED_ON);
        bool expansion = ny_expansion_active_.load(std::memory_order_acquire);
        ...
        if (!GlobalRiskGovernor::instance().canSubmitOrder(EngineId::CFD)) {
            std::cout << "[" << sessionName_ << "] ORDER BLOCKED - RISK GOVERNOR (daily loss or throttle)\n";
            return "";
        }
        
        // v4.5.1: NAS100 OWNERSHIP CHECK (final defense)
        if (symbol == "NAS100" && !canTradeNAS100(EngineId::CFD)) {
            std::cout << "[" << sessionName_ << "] ORDER BLOCKED - NAS100 OWNERSHIP VIOLATION\n";
            return "";
        }
