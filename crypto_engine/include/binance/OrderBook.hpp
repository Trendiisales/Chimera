...
public:
    ...
    // v7.12: For bookTicker stream - just update top of book (FASTEST)
    void set_top_of_book(double bid_price, double bid_qty,
                         double ask_price, double ask_qty) noexcept {
        bids_[0].price = bid_price;
        bids_[0].quantity = bid_qty;
        asks_[0].price = ask_price;
        asks_[0].quantity = ask_qty;
        // Don't touch other levels - they remain from last @depth20 snapshot
    }
