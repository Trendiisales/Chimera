...
public:
    ...
    OrderSender(
        ...
        , orders_rejected_(0)
        , blocked_orders_(0)
        , paper_fills_(0)  // v3.0: Paper mode fills
    {}
    ...
    void stop() noexcept {
        ...
        ws_.disconnect();  // Disconnect FIRST to unblock any reads
        if (thread_.joinable()) {
    ...
    [[nodiscard]] uint64_t paper_fills() const noexcept { return paper_fills_; }  // v3.0
...
private:
    ...
    void send_order(const OrderIntent& intent) noexcept {
        ...
        // Layer 1: Compile-time mode check (SHADOW/PAPER/LIVE)
        // Layer 2: Runtime paper mode check
        // Layer 3: API key has trading disabled on Binance side
        // ═══════════════════════════════════════════════════════════════════
        ...
        // Check if we're in SHADOW mode (no execution at all)
        ...
        // Check if we're in PAPER mode (simulate fills)
        if (is_paper_mode()) {
            // Paper mode uses ShadowExecutor - handled by SymbolThread
            // This code path should not be reached if properly integrated
            std::cout << "\n╔══════════════════════════════════════════════════════════╗\n";
            ...
            // Simulate immediate fill callback for GUI update
            if (on_fill_) {
                double fill_price = intent.price;  // Approximation
                on_fill_(intent.symbol_id, intent.side, intent.quantity, fill_price);
            }
            return;
        }
        
        // LIVE MODE - Check one more time
    ...
    uint64_t blocked_orders_;  // Orders blocked by SHADOW mode
    uint64_t paper_fills_;     // v3.0: Paper mode simulated fills
