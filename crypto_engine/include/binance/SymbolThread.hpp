...
public:
    ...
    // v4.9.34: Bootstrap probe notification methods
    void notify_probe_sent(uint64_t order_id, uint64_t sent_ns) noexcept {
        bootstrap_.probe_sent(order_id, sent_ns);
    }

    void notify_probe_ack(uint64_t order_id, uint64_t ack_ns, uint64_t latency_ns) noexcept {
        bootstrap_.probe_ack(order_id, ack_ns);
        double latency_ms = latency_ns / 1'000'000.0;
        std::cout << "[BOOTSTRAP_WIRE-" << config_.symbol << "] probe_ack notified: "
                  << "latency=" << latency_ms << "ms state=" << bootstrap_.state_string() << "\n";
    }

    void notify_probe_reject(uint64_t order_id, int error_code = -1, const char* reason = "rejected") noexcept {
        bootstrap_.probe_rejected(order_id, error_code, reason);
    }
...
private:
    ...
    void process_tick(const TickCore& tick) noexcept {
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // v4.9.3: LEGACY CRYPTO PATH DISABLED - MicroScalp is the ONLY crypto path
        // ═══════════════════════════════════════════════════════════════════════
        // This legacy SymbolThread execution path is DISABLED for crypto symbols.
        // All crypto trading now goes through CryptoMicroScalp which has:
        //   - Proper OFI-based edge calculation
        //   - Symbol-specific parameters (BTC/ETH/SOL)
        //   - Zero-qty guards
        //   - Proper logging
        // The shadow tracking above still runs for monitoring/ML purposes.
        // ═══════════════════════════════════════════════════════════════════════
        return;  // HARD BLOCK - MicroScalp handles all crypto execution
