...
struct CryptoParams {
    ...
    double min_spread_bps;      // Minimum spread to trade (maker protection)
    double max_spread_bps;      // Maximum spread to trade (cost protection)
    ...
    // v7.11: Testnet mode - relaxed depth/stability requirements
    bool testnet_mode;
    
    // Constructor with safe defaults (LIVE mode)
    CryptoParams()
        : base_size(0.0005)
        , max_position(1.0)
        , max_exposure_usd(50.0)
        , min_spread_bps(4.0)       // Minimum 4bps for maker edge
        , max_spread_bps(10.0)      // v7.11: Maximum 10bps (was implicit)
    ...
    // v7.11: Static factory for testnet configuration
    static CryptoParams testnet() {
        CryptoParams p;
        p.testnet_mode = true;
        p.min_spread_bps = 1.0;         // Relaxed - testnet spreads are weird
        p.max_spread_bps = 20.0;        // Allow wider spreads
        p.min_depth_usd = 100.0;        // Testnet has sparse depth
        p.max_toxic_flow = 0.90;        // Ignore toxicity on testnet
        p.max_vpin = 0.95;              // Ignore VPIN on testnet
        p.cooldown_ms = 500;            // Longer cooldown
        p.max_latency_us = 5000;        // Testnet is slow
        return p;
    }
};
...
private:
    ...
    bool can_trade(const CryptoTick& tick) const noexcept {
        ...
        double spread = tick.spread_bps();
        
        // v7.11: Spread gates
        // - min_spread_bps: Maker protection (too tight = adverse selection)
        // - max_spread_bps: Cost protection (too wide = guaranteed loss)
        if (spread < cfg_.params.min_spread_bps) return false;
        if (spread > cfg_.params.max_spread_bps) return false;
        
        // v7.11: Depth check (relaxed for testnet)
        if ((tick.bid_depth_5 + tick.ask_depth_5) < cfg_.params.min_depth_usd) return false;
        
        // v7.11: Skip toxicity/stability checks in testnet mode
        if (!cfg_.params.testnet_mode) {
    ...
    CryptoSide compute_signal(const CryptoTick& tick) const noexcept {
        ...
        // v7.11: Testnet mode - use simpler momentum signals
        // Depth imbalance doesn't work on testnet due to sparse books
        if (cfg_.params.testnet_mode) {
            // Simple momentum: if strong imbalance, trade in that direction
            if (imbalance > 0.3) {
                return CryptoSide::BID;  // Buy pressure
            }
            if (imbalance < -0.3) {
                return CryptoSide::ASK;  // Sell pressure
            }
            return CryptoSide::NONE;
        }
        
        // Live mode: Full depth analysis
