// =============================================================================
// PureScalper.hpp v6.98 - FIXED Trading Logic
// =============================================================================
// v6.98 FIXES:
//   - Mean reversion threshold: 0.2% -> 0.5% (less aggressive)
//   - Added volatility check for mean reversion
//   - TP/SL ratio: 15/20 -> 30/15 (better R:R)
//   - Max spread: 20 -> 10 bps (avoid wide spreads)
//   - Warmup: 15 -> 30 ticks
// =============================================================================
#pragma once
...
struct ScalpSignal {
    ...
    const char* reason = "";
    bool is_exit = false;
    double realized_pnl = 0.0;       // v6.99: Now in CURRENCY (not bps)
    double realized_pnl_bps = 0.0;   // v6.99: Keep bps for thresholds/debug
    double entry_price = 0.0;
    double exit_price = 0.0;
    MicroState micro_state = MicroState::IDLE;
    VetoReason veto_reason = VetoReason::NONE;
    
    bool shouldTrade() const { return direction != 0 && confidence >= 0.3; }
};
...
struct ScalpPosition {
    ...
    void update(double mid) { ticks_held++; highest = std::max(highest, mid); lowest = std::min(lowest, mid); }
...
struct SymbolState {
    ...
    int8_t trend() const { return (ema_fast > ema_slow && momentum > 0) ? 1 : 
...
public:
    struct Config {
        double size = 0.01;
        double tp_bps = 30.0;   // v6.98: Increased from 15 - need room to run
        double sl_bps = 15.0;   // v6.98: Tighter SL than TP - cut losses fast
        double trail_start = 10.0;  // v6.98: Start trailing at 10bps profit
        double trail_stop = 5.0;    // v6.98: Trail by 5bps
        double max_spread = 10.0;   // v6.98: Reduced from 20 - don't trade wide spreads
        int max_hold = 300;    // v6.98: Reduced from 500 - exit faster
        int warmup = 30;       // v6.98: Increased from 15 - more warmup
        bool debug = true;
        double contract_size = 100.0;  // v6.99: For currency PnL calculation
    };
    
    PureScalper() {
        microMgr_.setDebugSymbol("XAUUSD");
        microMgr_.setSimpleMode(true);  // Enable simple mode - bypasses impulse detection
    }
    ...
    ScalpSignal process(const char* symbol, double bid, double ask, double, double, uint64_t ts) {
        ...
        int8_t dir = st.trend();
        if (dir == 0) {
            // v6.98 FIX: Mean reversion was too aggressive (0.2% = 20bps)
            // Now requires 0.5% (50bps) deviation AND low volatility
            double dev = (st.mid - st.ema_slow) / st.ema_slow;
            double vol_z = st.micro_vol / st.ema_slow * 10000.0;  // Vol in bps
            
            // Only mean-revert if:
            // 1. Deviation > 0.5% (50 bps) - was 0.2%
            // 2. Volatility is low (< 5 bps) - don't fade during high vol
        ...
        // CRITICAL: Call onEntry
        micro.onEntry(dir, ts);
        
        if (cfg_.debug) {
            std::cout << "\n*** [SCALP] TRADE " << symbol << " " << sig.reason 
    ...
    ScalpSignal checkExit(SymbolState& st, uint64_t ts, MicroStateMachine& micro) {
        ...
        // v6.99: Calculate currency PnL: (exit - entry) * side * volume * contract_size
        ...
        if (pnl_bps >= cfg_.tp_bps) {
            ...
            sig.realized_pnl = calcCurrencyPnL();  // v6.99: Currency PnL
