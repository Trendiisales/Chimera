...
public:
    ...
    // Log entry decision (before trade)
    inline bool logEntry(
        ...
        float trend_str, uint16_t min_open, int8_t side, uint8_t strat_id
    ) noexcept {
        MLFeatureRecord rec;
        rec.timestamp_ns = ts;
        ...
        rec.intent = intent;
        rec.regime = regime;
        rec.atr_multiple = atr_mult;
        rec.volume_z = vol_z;
        rec.range_z = range_z;
        rec.distance_vwap = dist_vwap;
        rec.ofi = ofi;
        rec.vpin = vpin;
        rec.conviction_score = conviction;
        rec.spread_bps = spread_bps;
        rec.trend_strength = trend_str;
...
public:
    static bool exportToCSV(const char* binary_path, const char* csv_path) noexcept {
        ...
        // Header
        std::fprintf(fout, "timestamp_ns,symbol_id,state,intent,regime,"
                           "atr_mult,vol_z,range_z,dist_vwap,ofi,vpin,"
                           "conviction,spread_bps,trend_str,min_open,"
                           "side,strategy_id,realized_R,mfe_R,mae_R,hold_ms\n");
        ...
        while (std::fread(&rec, sizeof(MLFeatureRecord), 1, fin) == 1) {
            std::fprintf(fout, 
                ...
                rec.atr_multiple, rec.volume_z, rec.range_z, rec.distance_vwap,
                rec.ofi, rec.vpin,
                rec.conviction_score, rec.spread_bps, rec.trend_strength,
                (unsigned)rec.minutes_from_open,
                (int)rec.side, (unsigned)rec.strategy_id,
