...
// FIX #3: Lock striping for reduced contention
class LatencyEngineV2 {
public:
    explicit LatencyEngineV2(core::ThreadSafeQueue<ExecutionStats>& telemetry_queue);
    ...
    double get_quality_ema() const; // FIX #4: Feed back to adaptive

private:
    static constexpr int BUCKETS = 16;
    
    int get_bucket(const std::string& id) const;
    ...
    std::array<std::mutex, BUCKETS> m_bucket_mutex;
    ...
    std::atomic<double> m_quality_ema{0.0};
};
