...
LatencyEngineV2::LatencyEngineV2(core::ThreadSafeQueue<ExecutionStats>& telemetry_queue)
    : m_telemetry_queue(telemetry_queue) {}

// FIX #3: Hash-based bucket selection
int LatencyEngineV2::get_bucket(const std::string& id) const {
    return std::hash<std::string>{}(id) % BUCKETS;
}

void LatencyEngineV2::on_order_sent(const std::string& order_id, double mid_price, double spread) {
    int bucket = get_bucket(order_id);
    std::lock_guard<std::mutex> lock(m_bucket_mutex[bucket]);
...
void LatencyEngineV2::on_ack(const std::string& order_id) {
    int bucket = get_bucket(order_id);
    std::lock_guard<std::mutex> lock(m_bucket_mutex[bucket]);
...
void LatencyEngineV2::on_fill(const std::string& order_id, double fill_price) {
    ...
    update_ema(m_quality_ema, stats.quality_score, 0.1);
    
    m_telemetry_queue.push(stats);
}

double LatencyEngineV2::compute_quality_score(double slippage, double latency_ms, double spread) {
...
void LatencyEngineV2::update_ema(std::atomic<double>& ema, double value, double alpha) {
    double current = ema.load();
    double updated = alpha * value + (1.0 - alpha) * current;
    ema.store(updated);
}

double LatencyEngineV2::get_latency_ema() const {
    return m_latency_ema.load();
}

double LatencyEngineV2::get_slippage_ema() const {
    return m_slippage_ema.load();
}

double LatencyEngineV2::get_quality_ema() const {
    return m_quality_ema.load();
}
