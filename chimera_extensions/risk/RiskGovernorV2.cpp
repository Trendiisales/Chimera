...
RiskGovernorV2::RiskGovernorV2(double max_daily_loss, double base_spread_limit,
...
RiskDecision RiskGovernorV2::evaluate(const core::OrderIntent& intent) {
...
void RiskGovernorV2::update_market_state(double spread, double volatility, double latency_ms) {
...
void RiskGovernorV2::record_fill(double pnl) {
    m_daily_pnl.fetch_add(pnl);
}

void RiskGovernorV2::record_reject() {
    m_reject_count.fetch_add(1);
}

void RiskGovernorV2::record_latency(double latency_ms) {
    update_market_state(m_current_spread.load(), m_current_vol.load(), latency_ms);
}

void RiskGovernorV2::reset_daily() {
    m_daily_pnl.store(0.0);
    m_reject_count.store(0);
    m_session_anchor = std::chrono::steady_clock::now();
}

void RiskGovernorV2::set_kill_switch(bool state) {
    m_kill_switch.store(state);
}

SessionType RiskGovernorV2::detect_session() {
...
double RiskGovernorV2::compute_spread_threshold(SessionType s) {
    switch (s) {
...
double RiskGovernorV2::compute_vol_threshold(SessionType s) {
    switch (s) {
...
double RiskGovernorV2::compute_latency_threshold(SessionType s) {
    switch (s) {
...
double RiskGovernorV2::compute_drawdown_multiplier() {
    double dd_ratio = std::max(0.0, -m_daily_pnl.load() / m_max_daily_loss);
    return 1.0 - (dd_ratio * 0.5);
}

double RiskGovernorV2::compute_reject_penalty() {
    return std::max(0.5, 1.0 - (m_reject_count.load() * 0.03));
}

double RiskGovernorV2::compute_latency_penalty() {
...
double RiskGovernorV2::compute_volatility_penalty() {
