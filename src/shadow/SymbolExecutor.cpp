...
public:
    ...
    void update(double high, double low, double close) {
        highs_.push_back(high); lows_.push_back(low);
        if (highs_.size() > PERIOD) { highs_.pop_front(); lows_.pop_front(); }
        last_close_ = close;
    }
    
    double get() const {
        if (highs_.size() < 2) return 6.0;
        double sum = 0.0;
        for (size_t i = 0; i < highs_.size(); ++i) sum += highs_[i] - lows_[i];
        return sum / highs_.size();
    }
    
    double get_ref() const { return get() * 0.8; }
};

// === TRADE PERMISSION GATE ===
enum class TradeBlockReason { NONE, SESSION_NOT_ARMED, VOLATILITY_SHOCK, SYMBOL_MUTED, REJECT_FUSE, IMPULSE_NOT_PERSISTENT, ASIA_DISABLED };
...
struct TradeContext { const char* symbol; double impulse; double velocity; uint64_t now_ns; };

class TradePermissionGate {
public:
    static TradePermissionGate& instance() { static TradePermissionGate gate; return gate; }
    
    struct SymbolState {
        bool session_armed = false, volatility_shock = false, asia_disabled = false;
        uint64_t mute_until_ns = 0, reject_window_start_ns = 0, impulse_start_ns = 0;
        uint32_t rejects = 0, gate_blocks = 0;
    ...
    void onReject(const char* symbol) {
        ...
        if (s.rejects >= REJECT_LIMIT) { s.mute_until_ns = s.reject_window_start_ns + MUTE_NS; s.rejects = 0; std::cout << "[MUTE] " << symbol << "\n"; }
    }
    
    void onFill(const char* symbol) { auto& s = state(symbol); s.rejects = 0; std::cout << "[GATE] " << symbol << " FILL\n"; }
    void onSessionArm(const char* symbol) { state(symbol).session_armed = true; std::cout << "[GATE] " << symbol << " SESSION_ARMED\n"; }
    void onVolatilityShock(const char* symbol, bool active) { auto& s = state(symbol); bool changed = (s.volatility_shock != active); s.volatility_shock = active; if (changed && active) std::cout << "[GATE] " << symbol << " SHOCK ACTIVE\n"; else if (changed && !active) std::cout << "[GATE] " << symbol << " SHOCK CLEARED\n"; }
    void onAsiaDisable(const char* symbol, bool disabled) { auto& s = state(symbol); bool changed = (s.asia_disabled != disabled); s.asia_disabled = disabled; if (changed && disabled) std::cout << "[GATE] " << symbol << " ASIA_DISABLED\n"; else if (changed && !disabled) std::cout << "[GATE] " << symbol << " ASIA_ENABLED\n"; }
};
...
// === REGIME ===
enum class XAURegime { IDLE, CHOP, EXPANSION };
class XAURegimeClassifier {
public:
    XAURegime classify(double velocity, double spread) {
        if (std::abs(velocity) < 0.06 && spread < 0.30) return XAURegime::CHOP;
        if (std::abs(velocity) >= 0.12) return XAURegime::EXPANSION;
        return XAURegime::IDLE;
    }
};

// === IMPULSE FAILURE TRACKER ===
class ImpulseFailureTracker {
public:
    void on_impulse_attempt(Side side, uint64_t ts_ns) { last_impulse_side_ = side; last_impulse_ts_ = ts_ns; failed_ = false; }
    void on_impulse_fail() { failed_ = true; }
    
    bool eligible(uint64_t now_ns, double velocity) const {
        if (!failed_) return false;
        if (now_ns - last_impulse_ts_ > 200'000'000) return false;
        if (std::abs(velocity) > 0.04) return false;
        return true;
    }
    
    Side fade_side() const { 
        if (last_impulse_side_ == Side::BUY) return Side::SELL; 
        if (last_impulse_side_ == Side::SELL) return Side::BUY; 
        return Side::FLAT; 
    }
    void consume() { failed_ = false; }
    
private:
    Side last_impulse_side_ = Side::FLAT;
    uint64_t last_impulse_ts_ = 0;
    bool failed_ = false;
};

// === ENGINES ===
struct EngineIntent { Side side = Side::FLAT; double price = 0.0; double size = 0.0; const char* engine = "NONE"; };

class ChopReversionEngine {
public:
    EngineIntent evaluate(double velocity, double bid, double ask) {
        EngineIntent intent;
        if (velocity > 0.05) { intent.side = Side::SELL; intent.price = ask; intent.size = 0.01; intent.engine = "CHOP_REV"; }
        else if (velocity < -0.05) { intent.side = Side::BUY; intent.price = bid; intent.size = 0.01; intent.engine = "CHOP_REV"; }
        return intent;
    }
};

class ImpulseFailureReversion {
public:
    EngineIntent evaluate(uint64_t now_ns, double velocity, double bid, double ask, ImpulseFailureTracker& tracker) {
        EngineIntent intent;
        if (!tracker.eligible(now_ns, velocity)) return intent;
        Side s = tracker.fade_side();
        if (s == Side::FLAT) return intent;
        intent.side = s; intent.size = 0.005; intent.price = (s == Side::BUY) ? bid : ask; intent.engine = "IMP_FAIL";
        tracker.consume();
        return intent;
    }
};
...
public:
    EngineIntent evaluate(uint64_t now_ns, double velocity, double spread, double latency_ms, double bid, double ask) {
        EngineIntent intent;
        if (latency_ms > 6.0 || std::abs(velocity) > 0.02 || spread < 0.18) return intent;
        if (now_ns - last_emit_ts_ < 300'000'000) return intent;
        
        Side s = flip_side_ ? Side::BUY : Side::SELL;
        intent.side = s; intent.size = 0.005; intent.price = (s == Side::BUY) ? bid : ask; intent.engine = "MAKER";
        last_emit_ts_ = now_ns; flip_side_ = !flip_side_;
        return intent;
    }
    
private:
    bool flip_side_ = false;
    uint64_t last_emit_ts_ = 0;
};

// === IMPULSE EXPANSION ENGINE ===
...
public:
    uint64_t impulse_ok_since_ = 0, cooldown_until_ = 0;
    ...
    int compute_max_legs(double atr, bool asia) { int legs = int(atr / ATR_PER_LEG); if (legs < 1) legs = 1; if (legs > MAX_LEGS_HARD) legs = MAX_LEGS_HARD; if (asia) legs = 1; return legs; }
};
...
struct VolatilityShock {
    ...
    void update(double atr, double atr_ref, double impulse, double velocity, double latency_ms, uint64_t now_ns) {
        ...
        if (flags >= 2 && state == ShockState::NORMAL) { state = ShockState::SHOCK; shock_ns = now_ns; std::cout << "[SHOCK] DETECTED\n"; }
    }
...
struct SessionArmer { 
    ...
    void on_quote(uint64_t now_ns) { 
        if (first_quote_ns == 0) first_quote_ns = now_ns; 
        if (!armed && now_ns - first_quote_ns >= WARMUP_NS) { armed = true; armed_ns = now_ns; } 
    } 
...
struct TokyoRamp { 
    ...
    void on_session(bool is_tokyo, uint64_t now_ns) { 
        if (is_tokyo && !active) { open_ns = now_ns; active = true; } 
        if (!is_tokyo) { open_ns = 0; active = false; } 
    } 
    
    double size_scale(uint64_t now_ns) const { 
        if (!active) return 1.0; 
        double t = double(now_ns - open_ns) / double(RAMP_NS); 
        if (t <= 0.0) return 0.3; 
        if (t >= 1.0) return 1.0; 
        return 0.3 + 0.7 * t; 
    } 
    
    bool allow(uint64_t now_ns) const { 
        if (!active) return true; 
        return (now_ns - open_ns) > 120'000'000; 
    } 
};

struct AsiaTPDecay { 
    static constexpr uint64_t START_NS = 300'000'000, FULL_NS = 900'000'000; 
    double scale(uint64_t age_ns, bool asia) const { 
        if (!asia) return 1.0; 
        if (age_ns <= START_NS) return 1.0; 
        if (age_ns >= FULL_NS) return 0.4; 
        double t = double(age_ns - START_NS) / double(FULL_NS - START_NS); 
        return 1.0 - 0.6 * t; 
    } 
};
...
struct AsiaFailSafe { 
    ...
    void on_exit(double pnl, bool asia, bool london) { 
        if (asia && pnl < 0.0) { 
            losses++; 
            if (losses >= 2) { 
                disabled = true; 
                std::cout << "[ASIA] AUTO-DISABLED\n"; 
            } 
        } 
        if (london && disabled) { 
            losses = 0; 
            disabled = false; 
            std::cout << "[ASIA] RE-ARMED\n"; 
        } 
    } 
    
    void on_session_change(bool asia) { 
        if (!asia) { losses = 0; disabled = false; } 
    } 
...
struct LondonBoost { 
    ...
    double scale(uint64_t since_open_ns, bool london, bool fast) const { 
        if (!london || !fast) return 1.0; 
        if (since_open_ns > WINDOW_NS) return 1.0; 
        return 1.25; 
    } 
};
...
public: 
    ExecRegime regime = ExecRegime::FAST; 
    HaltControl halt; 
    
    void update_regime(uint64_t now_ns, double p95_ms) { 
        ExecRegime prev = regime; 
        if (p95_ms >= 200.0) regime = ExecRegime::HALT; 
        else if (p95_ms >= 20.0) regime = ExecRegime::SLOW; 
        else regime = ExecRegime::FAST; 
        if (regime != prev && regime == ExecRegime::HALT) { 
            halt.active = true; halt.entered_ns = now_ns; halt.trimmed = false; 
        } else if (regime == ExecRegime::FAST) { 
            halt.active = false; 
        } 
    } 
    ...
    bool should_trim_halt(uint64_t now_ns, double pnl, int legs, double& trim_frac) { 
        if (!halt.active || halt.trimmed || legs == 0) return false; 
        if (now_ns - halt.entered_ns < 500'000'000) return false; 
        if (pnl > -1.50) return false; 
        trim_frac = 0.50; halt.trimmed = true; 
        return true; 
    } 
    
    bool should_exit_chop(double pnl, double vel, int legs) const { 
        return (regime == ExecRegime::FAST && legs >= 2 && pnl <= -2.00 && std::fabs(vel) < 0.05); 
    } 
};
...
public: 
    ...
    void maybe_arm(uint64_t now_ns, int legs, int max_legs, double impulse, double velocity, double pnl, ExecRegime regime) { 
        if (regime != ExecRegime::FAST) return; 
        if (legs < max_legs) return; 
        if (impulse < 0.08 && std::fabs(velocity) < 0.07 && pnl < 0.50) { 
            if (!state.armed) { state.armed = true; state.armed_at_ns = now_ns; } 
        } else { 
            state.armed = false; 
        } 
    } 
    
    bool should_trim(uint64_t now_ns, double pnl, double& trim_frac) { 
        if (!state.armed) return false; 
        if (now_ns - state.armed_at_ns < 750'000'000) return false; 
        if (pnl <= -4.50) { trim_frac = 1.0; state.armed = false; return true; } 
        trim_frac = 0.66; state.armed = false; 
        return true; 
    } 
};
...
static XAURegimeClassifier g_regime_classifier;
static ImpulseFailureTracker g_impulse_failure;
static ChopReversionEngine g_chop_engine;
static ImpulseFailureReversion g_failure_engine;
static PassiveMakerEngine g_maker_engine;
...
void SymbolExecutor::onSignal(const Signal& s, uint64_t ts_ms) {
    ...
    double spread = last_ask_ - last_bid_;
    
    // PRE-FLIGHT GATE
    TradeContext ctx; 
    ctx.symbol = cfg_.symbol.c_str(); 
    ctx.impulse = impulse; 
    ...
    XAURegime regime = g_regime_classifier.classify(velocity, spread);
    
    EngineIntent intent;
    
    // ENGINE ROUTING (PRIORITY ORDER)
    if (regime == XAURegime::EXPANSION) {
        // IMPULSE EXPANSION ENGINE
        if (canEnter(s, ts_ms)) {
            g_impulse_failure.on_impulse_attempt(s.side, now_ns);
            intent.side = s.side;
            intent.price = (s.side == Side::BUY) ? last_ask_ : last_bid_;
            intent.size = 1.0;
            intent.engine = "IMPULSE";
        } else {
            g_impulse_failure.on_impulse_fail();
            TradePermissionGate::instance().onReject(cfg_.symbol.c_str());
            return;
        }
    }
    else if (regime == XAURegime::CHOP && legs_.empty()) {
        // CHOP ENGINES (PRIORITY ORDER)
        intent = g_failure_engine.evaluate(now_ns, velocity, last_bid_, last_ask_, g_impulse_failure);
        if (intent.side == Side::FLAT) intent = g_maker_engine.evaluate(now_ns, velocity, spread, last_latency_ms_, last_bid_, last_ask_);
        if (intent.side == Side::FLAT) intent = g_chop_engine.evaluate(velocity, last_bid_, last_ask_);
    }
    
    if (intent.side == Side::FLAT) return;
    
    // EXECUTE
    enterFromEngine(intent.side, intent.price, intent.size, intent.engine, ts_ms);
    TradePermissionGate::instance().onFill(cfg_.symbol.c_str());
}

void SymbolExecutor::enterFromEngine(Side side, double price, double base_size, const char* engine, uint64_t ts) {
    ...
    double stop_distance = (metal_type_ == Metal::XAU) ? 2.20 : 0.15;
    double tp_distance = (metal_type_ == Metal::XAU) ? 3.50 : 0.25;
    ...
    leg.entry_impulse = std::fabs(router_.get_velocity(cfg_.symbol)); 
    leg.entry_ts = ts;
    ...
    std::cout << "[" << cfg_.symbol << "] ENTRY [" << engine << "] trade_id=" << trade_id 
...
bool SymbolExecutor::canEnter(const Signal& s, uint64_t ts_ms) {
    ...
    double velocity = router_.get_velocity(cfg_.symbol);
    double impulse = std::fabs(velocity);
    bool asia = SessionClock::is_asia(now_ns);
    bool tokyo = SessionClock::is_tokyo(now_ns);
    bool london = SessionClock::is_london(now_ns);
    
    if (metal_type_ == Metal::XAU) {
        if (asia && !g_asia_failsafe.allow()) { rejection_stats_.total_rejections++; return false; }
        
        double atr = g_atr.get();
        double atr_ref = g_atr.get_ref();
        ...
        MarketState mkt; 
        mkt.impulse = impulse; 
        mkt.velocity = velocity; 
        mkt.atr = atr; 
        mkt.now_ns = now_ns; 
        mkt.shock = g_vol_shock.is_shock(); 
        mkt.asia_session = asia; 
        mkt.session_loaded = g_session_armer.allow(); 
        mkt.current_legs = current_legs;
    ...
    if (metal_type_ == Metal::XAG) {
        if (!london || current_legs >= 1 || impulse < 0.14 || velocity < 0.09) { 
...
void SymbolExecutor::enterBase(Side side, double price, uint64_t ts) { 
    enterFromEngine(side, price, 1.0, "LEGACY", ts); 
}
...
void SymbolExecutor::onTick(const Tick& t) {
    ...
    if (g_session_armer.allow() && !g_session_armer.notified) { 
        TradePermissionGate::instance().onSessionArm(cfg_.symbol.c_str()); 
        g_session_armer.notified = true; 
    }
    ...
    for (const auto& leg : legs_) { 
        ...
        double leg_pnl = is_long ? (current_price - leg.entry) * leg.size : (leg.entry - current_price) * leg.size; 
    ...
    if (asia && minutes_to_london <= 10 && current_legs > 0) { 
        exitAll("ASIA_END_FLATTEN", (t.bid + t.ask) / 2.0, t.ts_ms); 
        return; 
    }
    
    for (size_t i = 0; i < legs_.size(); ) {
        auto& leg = legs_[i]; 
        uint64_t age_ns = now_ns - (leg.entry_ts * 1'000'000);
        
        if (age_ns > 180'000'000) {
            double impulse_thresh = asia ? 0.04 : 0.08;
            double vel_thresh = asia ? 0.02 : 0.05;
        ...
        if (asia) { 
            uint64_t age_ns = now_ns - (leg.entry_ts * 1'000'000); 
            double tp_scale = g_asia_tp_decay.scale(age_ns, asia); 
            bool is_long = (leg.side == Side::BUY); 
            double original_tp = leg.take_profit; 
            double new_tp = is_long ? leg.entry + (original_tp - leg.entry) * tp_scale : leg.entry - (leg.entry - original_tp) * tp_scale; 
    ...
    if (metal_type_ == Metal::XAU) {
        ...
        if (g_failure.should_trim(now_ns, unrealized_pnl, trim_frac)) {
            int to_trim = static_cast<int>(legs_.size() * trim_frac); 
            if (trim_frac >= 1.0) to_trim = legs_.size();
            
            for (int i = 0; i < to_trim && !legs_.empty(); i++) { 
                double exit_price = (legs_[0].side == Side::BUY) ? t.bid : t.ask; 
                double pnl = (legs_[0].side == Side::BUY) ? (exit_price - legs_[0].entry) * legs_[0].size : (legs_[0].entry - exit_price) * legs_[0].size; 
                realized_pnl_ += pnl; 
                std::cout << "[" << cfg_.symbol << "] FAILURE_EXIT pnl=$" << pnl << "\n"; 
                g_asia_failsafe.on_exit(pnl, asia, london); 
                legs_.erase(legs_.begin()); 
                leg_to_trade_.erase(0); 
            }
    ...
    if (g_survival.should_exit_chop(unrealized_pnl, velocity, current_legs)) { 
        exitAll("CHOP_SHIELD", (t.bid + t.ask) / 2.0, t.ts_ms); 
        return; 
    }
    ...
    if (g_survival.should_trim_halt(now_ns, unrealized_pnl, current_legs, halt_trim_frac)) {
        ...
        for (int i = 0; i < to_trim && !legs_.empty(); i++) { 
            ...
            double pnl = (legs_[0].side == Side::BUY) ? (exit_price - legs_[0].entry) * legs_[0].size : (legs_[0].entry - exit_price) * legs_[0].size; 
            realized_pnl_ += pnl; 
            std::cout << "[" << cfg_.symbol << "] HALT_TRIM pnl=$" << pnl << "\n"; 
            legs_.erase(legs_.begin()); 
            leg_to_trade_.erase(0); 
        }
    }
    ...
    for (size_t i = 0; i < legs_.size(); ) {
        ...
        if (hit_stop || hit_tp) {
            ...
            if (gui_callback_) { 
                char side_char = (leg.side == Side::BUY) ? 'B' : 'S'; 
                gui_callback_(cfg_.symbol.c_str(), trade_id, side_char, leg.entry, exit_price, leg.size, pnl, t.ts_ms); 
            }
...
void SymbolExecutor::exitAll(const char* reason, double price, uint64_t ts) {
    ...
    for (size_t i = 0; i < legs_.size(); i++) { 
        auto& leg = legs_[i]; 
        uint64_t trade_id = leg_to_trade_[i]; 
        double pnl = (leg.side == Side::BUY) ? (price - leg.entry) * leg.size : (leg.entry - price) * leg.size; 
        realized_pnl_ += pnl; 
        ...
        if (exit_callback_) exit_callback_(cfg_.symbol.c_str(), trade_id, price, pnl, reason); 
        if (gui_callback_) { 
