...
#include "execution/ExecutionGovernor.hpp"  // v4.9.25: Venue state visibility
#include "bootstrap/LatencyBootstrapper.hpp"  // v4.9.27: (DEPRECATED - use ProbeController)
#include "execution/ProbeController.hpp"  // v4.9.27: Single-owner probe controller
...
int main() {
    ...
    // v4.9.32: Load credentials from config.ini FIRST
    if (!Chimera::Binance::initialize_binance_config()) {
        std::cerr << "[FATAL] Failed to load Binance credentials from config.ini\n";
        return 1;
    }
    ...
    g_gui.setVersion("v4.9.34");  // v4.9.34: CFD latency for HOT LAT, spike filter 500ms
    if (!g_gui.start()) {
    ...
    // =========================================================================
    // v4.9.27: PROBE CONTROLLER - Single-owner, ACK-gated, per-symbol state machine
    // =========================================================================
    // 
    // DESIGN RULES (non-negotiable):
    // ✓ One probe per symbol at a time
    // ✓ ACK-gated (structural, not advisory)
    // ✓ IOC timeInForce (auto-cancel)
    // ✓ Cancel before retry on timeout
    // ✓ Never parallel
    // ✓ Never burst
    // ✓ Only enable when engine is RUNNING
    //
    // Set to false to disable probes entirely for market-only testing
    constexpr bool ENABLE_PROBES = true;  // <-- SET TO false TO DISABLE PROBES
    
    auto& probe_ctrl = Chimera::getProbeController();
    
    if (ENABLE_PROBES) {
        // Register BTC only first - add others after BTC works
        // Register BTC only first - qty is fixed at 0.0001 internally
        probe_ctrl.registerSymbol("BTCUSDT");
        // probe_ctrl.registerSymbol("ETHUSDT");  // Enable after BTC works
        // probe_ctrl.registerSymbol("SOLUSDT");  // Enable after ETH works
        
        // Connect probe send callback to BinanceEngine
        // Uses IOC timeInForce for safe probes
        probe_ctrl.setSendCallback([&binance_engine](const char* symbol, double price, double qty, 
                                                      uint64_t client_id, bool use_ioc) -> bool {
            // Map symbol to ID
            uint16_t symbol_id = Chimera::Binance::symbol_to_id(symbol, strlen(symbol));
            return binance_engine.sendProbeOrder(symbol_id, symbol, price, qty, client_id, use_ioc);
        });
        
        // Connect cancel callback
        probe_ctrl.setCancelCallback([&binance_engine](const char* symbol, uint64_t exchange_order_id) -> bool {
            uint16_t symbol_id = Chimera::Binance::symbol_to_id(symbol, strlen(symbol));
            binance_engine.cancelProbeOrder(symbol_id, symbol, exchange_order_id);
            return true;
        });
        
        // Connect latency callback - just for logging
        // Note: BinanceOrderSender already records latency internally when ACK received
        probe_ctrl.setLatencyCallback([](const char* symbol, uint64_t latency_ns) {
            double latency_ms = latency_ns / 1'000'000.0;
            printf("[PROBE_LATENCY] %s: %.3fms (already recorded by BinanceOrderSender)\n", symbol, latency_ms);
        });
        
        // Wire ACK/reject callbacks FROM BinanceEngine TO ProbeController
        binance_engine.setProbeAckCallback([&probe_ctrl, &binance_engine](
            uint16_t symbol_id, uint64_t client_order_id, uint64_t exchange_order_id, uint64_t latency_ns) {
            
            printf("[PROBE-ACK] symbol_id=%u client_id=%llu exchange_id=%llu latency=%.3fms\n",
                   symbol_id,
                   static_cast<unsigned long long>(client_order_id),
                   static_cast<unsigned long long>(exchange_order_id),
                   latency_ns / 1'000'000.0);
            
            // Notify ProbeController for its internal tracking
            probe_ctrl.onOrderAck(client_order_id, exchange_order_id);
            
            // v4.9.34: THE MISSING LINK - Notify BootstrapEvaluator!
            uint64_t now_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                std::chrono::steady_clock::now().time_since_epoch()).count();
            binance_engine.notifyBootstrapProbeAck(symbol_id, client_order_id, now_ns, latency_ns);
        });
        
        binance_engine.setProbeCancelAckCallback([&probe_ctrl](uint64_t exchange_order_id) {
            probe_ctrl.onCancelAck(exchange_order_id);
        });
        
        binance_engine.setProbeRejectCallback([&probe_ctrl](uint64_t client_order_id) {
            printf("[PROBE-REJECT] client_id=%llu\n", static_cast<unsigned long long>(client_order_id));
            probe_ctrl.onOrderReject(client_order_id, -1, "rejected");
        });
        
        printf("[PROBE_CTRL] v4.9.30: Single-owner probe controller initialized\n");
        printf("[PROBE_CTRL] BTC only, IOC, 2s spacing, 3s timeout\n");
        printf("[PROBE_CTRL] ═══════════════════════════════════════════════\n");
        printf("[PROBE_CTRL] STARTUP DIAGNOSTICS:\n");
        printf("[PROBE_CTRL]   ENABLE_PROBES=%d\n", ENABLE_PROBES ? 1 : 0);
        printf("[PROBE_CTRL]   send_cb wired=%d\n", probe_ctrl.hasSendCallback() ? 1 : 0);
        printf("[PROBE_CTRL]   cancel_cb wired=%d\n", probe_ctrl.hasCancelCallback() ? 1 : 0);
        printf("[PROBE_CTRL]   symbols registered: BTCUSDT\n");
        printf("[PROBE_CTRL] ═══════════════════════════════════════════════\n");
    } else {
        printf("[PROBE_CTRL] *** PROBES DISABLED - Market-only mode ***\n");
    }
    ...
    // v4.9.28: SEPARATE probe tick interval - probes need isConnected(), NOT is_running()
    uint64_t last_probe_tick_ms = 0;
    constexpr uint64_t PROBE_TICK_INTERVAL_MS = 100;  // 10 ticks/sec to probe controller
    ...
    try {
        while (g_running && !g_kill.killed()) {
            ...
            try {
                ...
                // =============================================================
                // CRITICAL FIX: Probes are TELEMETRY, not trading
                // They only need WebSocket connection, NOT engine RUNNING state
                // The old code blocked probes because is_running() returns false
                // when engine is in INIT [NO_TRADE] state - this is WRONG for probes!
                // =============================================================
                if (ENABLE_PROBES && binance_engine.isConnected() && 
                    (now_ms - last_probe_tick_ms) >= PROBE_TICK_INTERVAL_MS) {
                    
                    last_probe_tick_ms = now_ms;
                    
                    // v4.9.30: COMPREHENSIVE PROBE DIAGNOSTICS
                    static uint64_t probe_diag_counter = 0;
                    probe_diag_counter++;
                    bool verbose = (probe_diag_counter % 100 == 1);  // Log every 10 seconds
                    
                    if (verbose) {
                        printf("[PROBE_DIAG] ═══════════════════════════════════════════════\n");
                        printf("[PROBE_DIAG] Probe block entered (iteration %llu)\n", 
                               static_cast<unsigned long long>(probe_diag_counter));
                        printf("[PROBE_DIAG] isConnected()=%d is_running()=%d\n",
                               binance_engine.isConnected() ? 1 : 0,
                               binance_engine.is_running() ? 1 : 0);
                    }
                    
                    // v4.9.29: Check time sync status - probes CANNOT work without it
                    static bool time_sync_warned = false;
                    static uint64_t last_time_sync_retry_ms = 0;
                    bool time_sync_ok = Chimera::Binance::BinanceTimeSync::is_initialized();
                    
                    if (verbose) {
                        printf("[PROBE_DIAG] time_sync_ok=%d offset=%lldms\n",
                               time_sync_ok ? 1 : 0,
                               static_cast<long long>(Chimera::Binance::BinanceTimeSync::offset_ms()));
                    }
                    
                    if (!time_sync_ok) {
                        // Log warning once
                        if (!time_sync_warned) {
                            printf("[PROBE_CTRL] ⚠️  TIME SYNC NOT COMPLETE - probes blocked until sync succeeds\n");
                            printf("[PROBE_CTRL] Will retry time sync every 10 seconds...\n");
                            time_sync_warned = true;
                        }
                        
                        // Retry time sync every 10 seconds
                        if (now_ms - last_time_sync_retry_ms >= 10000) {
                            last_time_sync_retry_ms = now_ms;
                            printf("[PROBE_CTRL] Retrying time sync...\n");
                            uint64_t server_time_ms = 0;
                            if (binance_engine.syncServerTime(server_time_ms)) {
                                printf("[PROBE_CTRL] ✓ Time sync recovered! Probes can now fire.\n");
                                time_sync_warned = false;  // Reset for future issues
                            } else {
                                printf("[PROBE_CTRL] ✗ Time sync retry failed - will try again in 10s\n");
                            }
                        }
                        // Skip probe ticks until time sync succeeds
                        goto skip_probe_ticks;
                    }
                    
                    // Enable probes when connected AND time synced
                    static bool probes_enabled = false;
                    if (!probes_enabled) {
                        probe_ctrl.enable();
                        probes_enabled = true;
                        printf("[PROBE_CTRL] ✓ Enabled (WS connected + time synced)\n");
                    }
                    
                    // Feed BTCUSDT tick to probe controller
                    {
                        uint16_t probe_btc_id = Chimera::Binance::symbol_to_id("BTCUSDT", 7);
                        const auto* probe_btc = binance_engine.get_symbol_thread(probe_btc_id);
                        
                        if (verbose) {
                            printf("[PROBE_DIAG] BTCUSDT: thread=%p tick_count=%llu\n",
                                   static_cast<const void*>(probe_btc),
                                   probe_btc ? static_cast<unsigned long long>(probe_btc->tick_count()) : 0ULL);
                        }
                        
                        if (probe_btc && probe_btc->tick_count() > 0) {
                            const auto& book = probe_btc->book();
                            double bid = book.best_bid();
                            double ask = book.best_ask();
                            
                            if (verbose) {
                                printf("[PROBE_DIAG] BTCUSDT: bid=%.2f ask=%.2f\n", bid, ask);
                            }
                            
                            if (bid > 0 && ask > 0) {
                                double mid = (bid + ask) / 2.0;
                                if (verbose) {
                                    printf("[PROBE_DIAG] BTCUSDT: calling onTick(mid=%.2f)\n", mid);
                                }
                                probe_ctrl.onTick("BTCUSDT", mid);
                            } else if (verbose) {
                                printf("[PROBE_DIAG] BTCUSDT: SKIPPED - bid/ask <= 0\n");
                            }
                        } else if (verbose) {
                            printf("[PROBE_DIAG] BTCUSDT: SKIPPED - no thread or tick_count=0\n");
                        }
                    }
                    
                    // Feed ETHUSDT tick to probe controller (if registered)
                    {
                        uint16_t probe_eth_id = Chimera::Binance::symbol_to_id("ETHUSDT", 7);
                        const auto* probe_eth = binance_engine.get_symbol_thread(probe_eth_id);
                        if (probe_eth && probe_eth->tick_count() > 0) {
                            const auto& book = probe_eth->book();
                            double bid = book.best_bid();
                            double ask = book.best_ask();
                            if (bid > 0 && ask > 0) {
                                double mid = (bid + ask) / 2.0;
                                probe_ctrl.onTick("ETHUSDT", mid);
                            }
                        }
                    }
                    
                    // Feed SOLUSDT tick to probe controller (if registered)
                    {
                        uint16_t probe_sol_id = Chimera::Binance::symbol_to_id("SOLUSDT", 7);
                        const auto* probe_sol = binance_engine.get_symbol_thread(probe_sol_id);
                        if (probe_sol && probe_sol->tick_count() > 0) {
                            const auto& book = probe_sol->book();
                            double bid = book.best_bid();
                            double ask = book.best_ask();
                            if (bid > 0 && ask > 0) {
                                double mid = (bid + ask) / 2.0;
                                probe_ctrl.onTick("SOLUSDT", mid);
                            }
                        }
                    }
                    
                    skip_probe_ticks:;  // v4.9.29: Jump here if time sync not ready
                }
                ...
                if (binance_engine.is_running() && 
                    ...
                    // v4.9.28: Probe enable and tick feeding moved to separate block above
                    // This block is now purely for GUI tick broadcasts
                    ...
                    if (btc) {
                        ...
                        if (bid > 0 && ask > 0) {
                            ...
                            // v4.9.28: probe_ctrl.onTick() moved to separate probe tick block above
                        }
                    }
                    
                    // Broadcast ETHUSDT
                    uint16_t eth_id = Chimera::Binance::symbol_to_id("ETHUSDT", 7);
                    const auto* eth = binance_engine.get_symbol_thread(eth_id);
                    ...
                    // Broadcast SOLUSDT
                    uint16_t sol_id = Chimera::Binance::symbol_to_id("SOLUSDT", 7);
                    const auto* sol = binance_engine.get_symbol_thread(sol_id);
                ...
                // THIS WAS MISSING - latency never wired to GUI!
                // ═══════════════════════════════════════════════════════════════
                {
                    ...
                    g_gui.updateHotPathLatency(
                        ...
                        snap.p99_ms(),
                        snap.sample_count,
                        snap.spikes_filtered,
                        lat_state,
                        exec_mode
                    );
                    
                    // Also update system mode with probe stats
                    ...
                    // v4.9.34: CFD FIX LATENCY - CO-LOCATED EDGE (London → cTrader)
                    // This is the REAL co-location advantage, NOT Binance!
                    // Uses FIX TestRequest/Heartbeat RTT - continuous measurement even without trades
                    {
                        size_t cfd_samples = cfd_engine.fixRttSamples();
                        const char* cfd_state = cfd_samples > 0 ? "ACTIVE" : "NO_DATA";
                        
                        g_gui.updateCfdLatency(
                            cfd_engine.fixRttMinMs(),
                            cfd_engine.fixRttAvgMs(),
                            cfd_engine.fixRttMaxMs(),
                            cfd_engine.fixRttAvgMs(),  // p50 = avg for now
                            cfd_engine.fixRttMaxMs(),  // p99 = max for now
                            cfd_samples,
                            cfd_state
                        );
                    }
                    
                    // v4.9.25: Update venue state for FROZEN visibility
                    // v4.9.27: Added signature rejection counter + governor tick for alarm dampening
                    ...
                    const char* frozen_syms = any_frozen ? "ALL" : "";
                    uint32_t failures = static_cast<uint32_t>(gov.consecutive_issues());
                    uint64_t sig_rejects = gov.signature_rejections();
                    
                    g_gui.updateVenueState(venue_str, any_frozen, frozen_syms, failures, sig_rejects);
                    
                    // v4.9.27: Tick governor for alarm dampening timers + symbol unfreeze
                    gov.tick();
                }
