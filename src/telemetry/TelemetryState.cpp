...
void TelemetryState::increment_position_block() {
    position_blocks_.fetch_add(1);
}

uint64_t TelemetryState::position_blocks() const {
    return position_blocks_.load();
}
...
std::string TelemetryState::to_json() const {
    ...
    out << "{\"uptime\":" << uptime_sec_
        ...
        << ",\"position_blocks\":" << position_blocks_.load()
...
std::string TelemetryState::to_prometheus() const {
    ...
    out << "chimera_uptime " << uptime_sec_ << "\n"
        ...
        << "chimera_position_blocks " << position_blocks_.load() << "\n";
