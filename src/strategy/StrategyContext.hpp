...
public:
    StrategyContext(std::shared_ptr<Context> ctx, ExecutionRouter& router)
    ...
    // ---------------------------------------------------------------------------
    // REDUCE-ONLY SUBMISSION — bypasses position cap if truly reducing position.
    // Used by unwinders to break saturation safely. Only allowed if order
    // reduces abs(position).
    // ---------------------------------------------------------------------------
    bool submit_reduce_only(const std::string& client_id,
                           ...
                           double qty,
                           const std::string& engine_id) {
        return router_.submit_reduce_only(client_id, sym, price, qty, engine_id);
    }
