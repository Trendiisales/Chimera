    ...
    while (running.load()) {
        ...
        for (int i = 0; i < N_SYMBOLS; ++i) {
            ...
            for (const auto& intent : intents) {
                ...
                // Check if this is a reduce-only order (e.g. forced unwinding)
                if (intent.reduce_only) {
                    accepted = ctx_.submit_reduce_only(cid, intent.symbol,
                                                       intent.price, signed_qty,
                                                       intent.engine_id);
                } else {
                    accepted = ctx_.submit(cid, intent.symbol,
                                          intent.price, signed_qty,
                                          intent.engine_id);
                }
                ...
                if (accepted) {
                    std::cout << "[STRAT] " << engine_->id()
                              ...
                              << " id=" << cid
                              << (intent.reduce_only ? " [REDUCE-ONLY]" : "") << "\n";
                }
            }
        }
