...
void QueueMarketMaker::onTick(const MarketTick& tick, std::vector<OrderIntent>& out) {
    ...
    // INVENTORY MEAN REVERSION - actively close positions to realize PnL
    // Close aggressively when position > 5% of cap (0.05 with 0.30 cap = 16.7%)
    if (abs_pos > 0.05) {
        OrderIntent o;
        o.engine_id = engine_id_;
        o.symbol = tick.symbol;
        o.is_buy = (pos < 0);  // Buy if short, sell if long
        o.price = o.is_buy ? ask : bid;  // Cross spread to close immediately
        o.size = std::min(BASE_QTY, abs_pos);
        out.push_back(o);
        last_submit_ns_ = now;
        return;  // Don't open new positions while unwinding
    }
