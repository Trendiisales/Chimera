...
ExecutionRouter::ExecutionRouter(std::shared_ptr<Context> ctx)
    ...
    // Initialize SymbolState array (Tier 4/5)
    for (int i = 0; i < MAX_SYMBOLS; ++i) {
        symbols_[i].set_cap(0.05);
        symbols_[i].set_position(0.0);
    }
    
    elastic_.set_base_cap("BTCUSDT", 0.05);
    elastic_.set_base_cap("ETHUSDT", 0.05);
    elastic_.set_base_cap("SOLUSDT", 0.05);
    
    std::cout << "[ROUTER] Initialized with Tier 4/5 architecture (symbol suppression enabled)" << std::endl;
    // start_tier1();  // Keep disabled - deterministic path is better
}
...
bool ExecutionRouter::submit_order(const std::string& client_id,
    ...
    // ===========================================================================
    // TIER1 FAST PATH: If enabled, use lock-free submission
    // ===========================================================================
    if (tier1_enabled_) {
        TradeSignal sig{};
        ...
        return ring_.push(sig);  // Lock-free!
    }
    
    // ===========================================================================
    // ORIGINAL PATH: Mutex-based (when Tier1 not enabled)
    // ===========================================================================
    ...
    // ---------------------------------------------------------------------------
    // COOLDOWN CHECK — Negative Acknowledgement Backpressure
    // If engine+symbol was recently blocked, reject silently to prevent spam.
    // Standard HFT pattern: prevent strategies from DOSing their own router.
    // ---------------------------------------------------------------------------
    if (check_cooldown(engine_id, symbol)) {
        return false;  // Silent reject during cooldown
    }

    // ---------------------------------------------------------------------------
    // ATOMIC POSITION GATE + PORTFOLIO EDGE GATE
    // Two-layer filter:
    // 1. Edge must exceed dynamic floor (higher near cap)
    // 2. Position must not exceed cap
    // Lock held for atomic check of both.
    // ---------------------------------------------------------------------------
    {
        ...
        double util = std::fabs(current_pos) / MAX_POSITION;
        
        // Portfolio Edge Gate: dynamic edge floor based on utilization
        // Near cap = require much higher edge to trade
        // This prevents cap-hovering churn and fee bleeding
        // Note: edge_bps would come from strategy, for now we skip this check
        // since strategies don't pass edge yet. To enable:
        // 1. Modify submit_order signature to include edge_bps
        // 2. Uncomment the block below
        /*
        double base_edge = base_edge_bps_.count(symbol) 
                          ? base_edge_bps_[symbol] 
                          : 1.0;
        double required_edge = base_edge * (1.0 + util * cap_k_);
        
        if (edge_bps < required_edge) {
            rate_log("[EDGE_GATE] " + engine_id + " " + symbol +
                     " edge=" + std::to_string(edge_bps) +
                     " < req=" + std::to_string(required_edge) +
                     " util=" + std::to_string(util));
            set_cooldown(engine_id, symbol);
        ...
        */
        ...
        if (std::fabs(next_pos) > MAX_POSITION) {
            rate_log("[POSITION_GATE] BLOCK " + engine_id + " " + symbol 
                      + " would exceed cap: current=" + std::to_string(current_pos) 
                      + " + qty=" + std::to_string(qty) + " = " + std::to_string(next_pos));
            set_cooldown(engine_id, symbol);
            ctx_->telemetry.increment_risk_block();
            return false;
        }
