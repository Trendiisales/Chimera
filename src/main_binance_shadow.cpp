...
#include "execution/TierAllRouter.hpp"
...
int main() {
    ...
    // ---- CONTEXT: single owner of all state (shared ownership for thread safety) ----
    ...
    ctx->pnl.set_strategy_floor(-10.0);
    ctx->pnl.set_portfolio_dd(-500.0);
    ...
    ProfitPreset profit;
    ctx->profit = &profit;
    profit.reload_if_due();  // Load initial settings
    std::cout << "[CHIMERA] Profit preset: "
              << "max_pos=" << profit.max_position.load(std::memory_order_relaxed)
              << " suppress_ms=" << (profit.suppress_ns.load(std::memory_order_relaxed) / 1000000)
              << " min_queue=" << profit.min_queue_score.load(std::memory_order_relaxed)
              << "\n";
    ...
    // ---- COMPONENTS (now using shared Context) ----
    TierAllRouter   router(ctx);
    ...
    BTCascade            eng_btc;
    ETHSniper            eng_eth;
    MeanReversion        eng_mean_btc("BTCUSDT");
    MeanReversion        eng_mean_eth("ETHUSDT");
    MeanReversion        eng_mean_sol("SOLUSDT");
    QueueMarketMaker     eng_qmm;
    ImpulseReversion     eng_imp("BTCUSDT");
    ...
    StrategyRunner  run_mean_btc(&eng_mean_btc, strat_ctx);
    StrategyRunner  run_mean_eth(&eng_mean_eth, strat_ctx);
    StrategyRunner  run_mean_sol(&eng_mean_sol, strat_ctx);
    ...
    ThreadModel strat_mean_btc_thread(1, [&]() {
        run_mean_btc.run(ctx->running);
    });

    ThreadModel strat_mean_eth_thread(1, [&]() {
        run_mean_eth.run(ctx->running);
    });

    ThreadModel strat_mean_sol_thread(1, [&]() {
        run_mean_sol.run(ctx->running);
    });
    ...
    strat_mean_btc_thread.start();
    strat_mean_eth_thread.start();
    strat_mean_sol_thread.start();
    ...
    strat_mean_btc_thread.stop();
    strat_mean_eth_thread.stop();
    strat_mean_sol_thread.stop();
    ...
    strat_mean_btc_thread.join();
    strat_mean_eth_thread.join();
    strat_mean_sol_thread.join();
