    ...
    while (running.load()) {
        try {
            ...
            if (running.load()) {
                ...
                // In shadow mode, backoff is long (60s). Break into 1s chunks
                // so we can check running flag and exit quickly on shutdown.
                int backoff_seconds = ctx_.arm.live_enabled() ? 2 : 60;
                for (int i = 0; i < backoff_seconds && running.load(); ++i) {
