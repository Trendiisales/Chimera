...
param(
    [string]$LogRoot = "C:\Chimera\build\Release\logs",
    [int]$Days = 5
)
...
$tradeFiles = Get-RecentFiles "$LogRoot\trades\chimera_trade_closes_*.csv" $Days
if (-not $tradeFiles) {
    $fallback = "$LogRoot\trades\chimera_trade_closes.csv"
    if (Test-Path $fallback) { $tradeFiles = @(Get-Item $fallback) }
}

$shadowSignalFiles = Get-RecentFiles "$LogRoot\shadow\signals\chimera_shadow_signals_*.csv" $Days
if (-not $shadowSignalFiles) {
    $fallback = "$LogRoot\shadow\chimera_shadow_signals.csv"
    if (Test-Path $fallback) { $shadowSignalFiles = @(Get-Item $fallback) }
}

$shadowSignalEventFiles = Get-RecentFiles "$LogRoot\shadow\events\chimera_shadow_signal_events_*.csv" $Days
if (-not $shadowSignalEventFiles) {
    $fallback = "$LogRoot\shadow\chimera_shadow_signal_events.csv"
    if (Test-Path $fallback) { $shadowSignalEventFiles = @(Get-Item $fallback) }
}

$logFiles = Get-RecentFiles "$LogRoot\chimera_*.log" $Days

if (-not $tradeFiles -and -not $shadowSignalFiles -and -not $shadowSignalEventFiles -and -not $logFiles) {
    Write-Host "[ERROR] No Chimera logs found under $LogRoot" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "=== CHIMERA DIAGNOSIS ===" -ForegroundColor Cyan
...
if ($logLines.Count -gt 0) {
    foreach ($line in $logLines) {
        ...
        if ($line -match '\[CHIMERA-SECURITY\]\s+learned ext id\s+(\S+)\s+->\s+(\d+)') {
            $learnedExt[$matches[1]] = $matches[2]
        }
        if ($line -match '\[CHIMERA-MD\]\s+Unknown string symbol') {
            $unknownSymbols += $line
        }
        if ($line -match '\[CHIMERA-MD\]\s+Unknown numeric ID') {
            $unknownSymbols += $line
        }
    }
